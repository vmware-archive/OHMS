...
{
	loginfo "Copying working configurations to new dir"

	#cp -r -f "$BACKUP_DIR_FULLPATH/config/" $UPGRADE_CONFIG_DIR || {
	cp -r -f "$BACKUP_DIR_FULLPATH/config/" $UPGRADE_DIR_FULLPATH || {
		echo "Unable to copy exiting configurations!!!";
		return 157; }

	loginfo "Configurations copied Successfully"
}

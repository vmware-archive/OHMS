 ...
 */
@Controller
@RequestMapping( value = { "/upgrade" }, produces = { MediaType.APPLICATION_JSON_VALUE } )
public class UpgradeRestService
{
    ...
    @Value( "${hms.oob.upgrade.dir:/opt/vrack/upgrade}" )
    ...
    @ResponseBody
    public ResponseEntity<PrimitiveUpgradeStatus> upgrade( @RequestBody UpgradeSpec upgradeSpec,
                                                           HttpServletRequest request )
    {
        ...
        if ( !UpgradeUtil.validateUpgradeParameters( upgradeSpec ) )
        {
            ...
            primitiveUpgradeStatus =
                UpgradeUtil.getPrimitiveUpgradeStatus( upgradeId, UpgradeStatusCode.HMS_UPGRADE_INVALID_REQUEST,
                                                       PrimitiveUpgradeStatusCodes.COMPLETED_WITH_FAILURE );
            // return BAD_REQUEST
            ...
            && !UpgradeUtil.validateVersion( upgradeSpec.getPreviousVersion(), upgradeSpec.getPatchFile() ) )
        {
            ...
            primitiveUpgradeStatus =
                UpgradeUtil.getPrimitiveUpgradeStatus( upgradeId, UpgradeStatusCode.HMS_UPGRADE_FORBIDDEN,
                                                       PrimitiveUpgradeStatusCodes.COMPLETED_WITH_FAILURE );
            // return FORBIDDEN
            return new ResponseEntity<PrimitiveUpgradeStatus>( primitiveUpgradeStatus, HttpStatus.FORBIDDEN );
        }
        /*
         * LCM will send the SHA-1 checksum of the bundle. Check that the bundle exists and validate its checksum.
         */
        String bundleChecksum = FileUtil.getFileChecksum( upgradeSpec.getPatchFile(), ChecksumMethod.SHA1 );
        if ( bundleChecksum == null )
        {
            message = String.format( "Unable to compute checksum of the upgrade bundle - '%s' using algoritham - '%s'.",
                                     upgradeSpec.getPatchFile(), ChecksumMethod.SHA1.toString() );
            logger.error( message );
            primitiveUpgradeStatus =
                UpgradeUtil.getPrimitiveUpgradeStatus( upgradeId, UpgradeStatusCode.HMS_UPGRADE_INTERNAL_ERROR,
                                                       PrimitiveUpgradeStatusCodes.COMPLETED_WITH_FAILURE );
            // return INTERNAL_SERVER_ERROR
            return new ResponseEntity<PrimitiveUpgradeStatus>( primitiveUpgradeStatus,
        ...
        else
        {
            if ( !bundleChecksum.equalsIgnoreCase( upgradeSpec.getSha1CheckSum() ) )
            {
                message = String.format(
                                         "Upgrade bundle checkusm validation failed. "
        ...
        if ( !UpgradeUtil.notifyPRMService( ServiceState.NORMAL_MAINTENANCE, prmUserName, prmPassword ) )
        {
            // delete bundleExtractedDirAbsPath
            if ( !FileUtil.deleteDirectory( bundleExtractedDirAbsPath ) )
            {
        ...
        HmsUpgradeTask hmsUpgradeTask =
            hmsUpgradeTaskBuilder.upgradeSpec( upgradeSpec ).hmsUpgradeDir( hmsUpgradeDir ).bundleExtractedDirAbsPath( bundleExtractedDirAbsPath ).hmsUpgradeScript( hmsUpgradeScript ).hmsBackupDir( hmsBackupDir ).shutdownMonitoringMaxWaittime( shutdownMonitoringMaxWaittime ).serviceMaintenanceMaxWaittime( serviceMaintenanceMaxWaittime ).serviceMaintenanceRetryInterval( serviceMaintenanceRetryInterval ).hmsIbInventoryLocation( hmsIbInventoryLocation ).oobAgentUpgradeBundleAbsPath( oobAgentUpgradeBundleAbsPath ).oobAgentHost( oobAgentHost ).oobAgentPort( oobAgentPort ).oobUpgradeDir( oobUpgradeDir ).oobUpgradeMaxTimeout( oobUpgradeMaxTimeout ).oobUpgradeRetryInterval( oobUpgradeRetryInterval ).oobNodesEndpoint( oobNodesEndpoint ).prmPassword( prmPassword ).prmUserName( prmUserName ).build();
        ...
        boolean upgradeStatusFileSaved =
            HmsUpgradeUtil.saveUpgradeStatus( upgradeStatusFileNameAbsPath, upgradeStatus );
        ...
        primitiveUpgradeStatus =
            UpgradeUtil.getPrimitiveUpgradeStatus( upgradeId, UpgradeStatusCode.HMS_UPGRADE_INITIATED,
                                                   PrimitiveUpgradeStatusCodes.IN_PROGRESS, upgadeStatusURI );
        // return ACCEPTED
    ...
    public ResponseEntity<PrimitiveUpgradeStatus> getUpgradeStatus( @PathVariable( "upgradeId" ) String upgradeId)
    {
        ...
        {
            PrimitiveUpgradeStatusCodes primitiveUpgradeStatusCode = PrimitiveUpgradeStatusCodes.IN_PROGRESS;
            switch ( upgradeStatus.getStatusCode() )
            {
                ...
                default:
                    /*
                     * HMS Upgrade Status anything other than INITIATED or SUCCESS is an ERROR
                     */

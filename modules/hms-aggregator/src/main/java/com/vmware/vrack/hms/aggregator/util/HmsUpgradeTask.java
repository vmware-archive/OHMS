...
// * @author VMware, Inc.
// */
// public class HmsUpgradeTask implements Callable<Void> {
//
...
// */
// public HmsUpgradeTask(PrimitiveUpgradeSpec upgradeSpec, String hmsUpgradeDir, String bundleExtractedDirAbsPath,
// String hmsUpgradeScript, String hmsBackupDir, Long shutdownMonitoringMaxWaittime,
// int serviceMaintenanceMaxWaittime, int serviceMaintenanceRetryInterval, String hmsIbInventoryLocation,
// String oobAgentUpgradeBundleAbsPath, String oobAgentHost, int oobUpgradeMaxTimeout,
// int oobUpgradeRetryInterval, String oobNodesEndpoint, String prmHost, String prmUserName,
// String prmPassword, String hmsLocalUpgradeBundleAbsPath, String vrmTruststoreFile,
// String vrmTruststorePassword, String pscCa1Alias, String pscCa2Alias) {
//
...
// upgradeStatusFileNameAbsPath = String.format("%1$s/%2$s.json", this.hmsUpgradeDir, upgradeSpec.getUpgradeId());
...
//
// boolean hmsUpgradeScriptsStaged = this.stageUpgradeScripts();
//
// if (!hmsUpgradeScriptsStaged) {
//
...
//
// /*
// * We are not checking OOB Agent version before and after upgrade.
// * To determine whether upgrade succeeded or failed, we are
// * depending on the status returned by upgrade monitoring api of the
// * oob agent.
// */
...
// }
//
// UpgradeStatus oobUpgradeStatus = UpgradeUtil.getOobUpgradeStatus(upgradeSpec.getUpgradeId(),
...
//
// /*
// * if oobUpgradeStatus is null, that means something went wrong with
// * OOB Upgrade and needs manual intervention to bring it into
// * running state. UpgradeStatus should reflect this fact.
// */
...
// if (oobUpgradeStatusCode != null && !oobUpgradeStatusCode.equals(UpgradeStatusCode.HMS_OOB_UPGRADE_SUCCESS)) {
//
// // delete bundleExtractedDirAbsPath
// if (!FileUtil.deleteDirectory(bundleExtractedDirAbsPath)) {
// logger.warn("Failed to delete HMS upgrade bundle extracted directory -'{}'.",
...
//
// // vrm truststore path
// cmdWithArgs.add(7, vrmTruststoreFile);
//
// // vrm truststore password
// cmdWithArgs.add(8, vrmTruststorePassword);
//
// // psc ca 1 alias
// cmdWithArgs.add(9, pscCa1Alias);
//
// // psc ca 2 alias
// cmdWithArgs.add(10, pscCa2Alias);
//
...
// *
// * @param upgradeStatus the new hms in running
// */
// private void setHmsInRunning(final UpgradeStatus upgradeStatus) {
//
// // PUT BACK SERVICE IN RUNNING STATE
// boolean running = ServiceManager.setServiceInRunning(hmsIbInventoryLocation,
// oobNodesEndpoint);
// if (running) {
//
...
// if (!UpgradeUtil.notifyPRMService(ServiceState.RUNNING, prmHost, prmUserName, prmPassword)) {
...
// } else {
// /*
// * Monitoring API should reflect the status that HMS needs
// * manual intervention to bring it back to normal.
// */
// upgradeStatus.setStatusCode(UpgradeStatusCode.HMS_UPGRADE_FAILED_RESTART_REQUIRED);
...
// uploaded = UpgradeUtil.uploadFileToManagementSwitch(oobAgentUpgradeBundleAbsPath, upgradeSpec.getUpgradeId());
// if (!uploaded) {
// logger.error("Unable to upload OOB Agent upgrade bundle '{}' to OOB Agent host - {}.",
// oobAgentUpgradeBundleAbsPath, oobAgentHost);
// return false;
// }
//
// // Upload upgrade scripts to oob agent host
// uploaded = UpgradeUtil.uploadOobAgentUpgradeScriptsToOobAgentHost(bundleExtractedDirAbsPath, oobAgentHost,
// oobAgentPort, upgradeSpec.getUpgradeId());
// if (!uploaded) {
...
// String oobAgentUpgradeBundleChecksum = FileUtil.getFileChecksum(oobAgentUpgradeBundleAbsPath,
// ChecksumMethod.SHA1);
// oobUpgradeSpec.setChecksum(oobAgentUpgradeBundleChecksum);
// boolean oobAgentUpgradeInitiated = UpgradeUtil.initiateOobUpgrade(oobUpgradeSpec);
// if (!oobAgentUpgradeInitiated) {
...
// * @return true, if successful
// */
// public boolean stageUpgradeScripts() {
//
// // STEP#7: copy aggregator upgrade scripts to bundleExtractedDirAbsPath
// boolean hmsUpgradeScriptsCopied = UpgradeUtil.stageUpgradeScripts(hmsLocalUpgradeBundleAbsPath,
// bundleExtractedDirAbsPath);
//

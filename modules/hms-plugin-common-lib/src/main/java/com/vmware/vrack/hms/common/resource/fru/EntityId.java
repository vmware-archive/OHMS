...
{
    Unspecified( EntityId.UNSPECIFIED ),
    Other( EntityId.OTHER ),
    Unknown( EntityId.UNKNOWN ),
    Processor( EntityId.PROCESSOR ),
    /**
     * Disk or disk bay.
     */
    Disk( EntityId.DISK ),
    ...
    // D0h-FFh i.e 208 to 255 Entity ID for OEM
    private static final int OEM = 208;
    ...
    public static EntityId parseInt( int value )
    {
        switch ( value )
        {
            ...
            case BASEBOARD:
                return Baseboard;
            default:
                // D0h-FFh i.e 208 to 255 Entity ID for OEM; check if it's >= 208
                if ( value >= OEM )
                {
                    return Oem;
                }

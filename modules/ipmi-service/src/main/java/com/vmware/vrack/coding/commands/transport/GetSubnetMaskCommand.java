...
/**
 * Wrapper class for Get Subnet Mask of BMC request.
 * 
 * @author Yagnsesh Chawda
 */
public class GetSubnetMaskCommand
    extends IpmiCommandCoder
{
    private byte[] parameter;

    /**
     * Initiates GetSubnetMaskCommand for encoding and decoding. Here parameter variable expects byte array of size 4.
     * 1st byte - Channel number like 0x01. IF the most significant bit is 1 then it will fetch parameter revision ONLY
     * 2nd byte - Parameter selector number . refer Ipmi Specification. 3rd byte - Set Selector. If not implemented set
     * as 0x00 4th byte - block selector. if not implemented set as 0x00
     *
     ...
     */
    public GetSubnetMaskCommand( IpmiVersion version, CipherSuite cipherSuite, AuthenticationType authenticationType,
                                 byte[] parameter )
    {
    ...
    /**
     * GetCommandCode to return the Custom command Here Get Subnet Mask is a parameter function of Get LAN Configuration
     */
    @Override
        ...
        throws IllegalArgumentException, IPMIException, NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        GetSubnetMaskCommandResponseData responseData = new GetSubnetMaskCommandResponseData();
        ...
        // If parameter[0]'s MS bit is 0 then only it will fetch Subnet Mask
        if ( ( parameter != null ) && ( ( parameter[0] & TypeConverter.intToByte( 0x80 ) ) == 0 ) )
        {
            ...
            System.arraycopy( raw, 1, subnetMask, 0, 4 );
            responseData.setSubnetMask( subnetMask );
        }
        return responseData;
    }
}

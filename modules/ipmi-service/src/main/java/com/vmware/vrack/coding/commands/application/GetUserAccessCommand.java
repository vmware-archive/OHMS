...
/**
 * Wrapper class for Get User Access.
 * 
 * @author Yagnesh Chawda
 */
public class GetUserAccessCommand
    extends IpmiCommandCoder
{
    private byte[] parameter;

    /**
     * Initiates GetUserAccess for encoding and decoding.
     *
     ...
     */
    public GetUserAccessCommand( IpmiVersion version, CipherSuite cipherSuite, AuthenticationType authenticationType,
                                 byte[] param )
    {
    ...
    {
        return IpmiCommandCodes.GET_USER_ACCESS;
    }
        ...
        throws IllegalArgumentException, IPMIException, NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        GetUserAccessCommandResponseData responseData = new GetUserAccessCommandResponseData();
        responseData.setMaxUserIdCode( raw[0] );
        ;
        responseData.setCurrentlyEnabledUsersCountCode( raw[1] );
        responseData.setUserIdWithFixedNameCountCode( raw[2] );
        responseData.setChannelAccessCode( raw[3] );
        return responseData;
    }
}

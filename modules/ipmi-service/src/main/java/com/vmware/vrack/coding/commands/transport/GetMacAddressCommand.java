...
/**
 * Wrapper class Get MAC Address of BMC request.
 * 
 * @author Yagnsesh Chawda
 */
public class GetMacAddressCommand
    extends IpmiCommandCoder
{
    private byte[] parameter;

    /**
     * Initiates GetMacAddressCommand for encoding and decoding. Here parameter variable expects byte array of size 4.
     * 1st byte - Channel number like 0x01. IF the most significant bit is 1 then it will fetch parameter revision ONLY
     * 2nd byte - Parameter selector number . refer Ipmi Specification.For Mac Address it is 0x05 3rd byte - Set
     * Selector. If not implemented set as 0x00 4th byte - block selector. if not implemented set as 0x00
     *
     ...
     */
    public GetMacAddressCommand( IpmiVersion version, CipherSuite cipherSuite, AuthenticationType authenticationType,
                                 byte[] parameter )
    {
    ...
    /**
     * GetCommandCode to return the Custom command Here Get Mac Address is a parameter function of Get LAN Configuration
     */
    @Override
        ...
        throws IllegalArgumentException, IPMIException, NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        GetMacAddressCommandResponseData responseData = new GetMacAddressCommandResponseData();
        ...
        if ( ( parameter != null ) && ( ( parameter[0] & TypeConverter.intToByte( 0x80 ) ) == 0 ) )
        {
            ...
            responseData.setMacAddress( macAddress );
        }
        return responseData;
    }
}

...
/**
 * Power Down Server Task to trigger power down server via BMC
 * 
 * @author Vmware
 */
public class PowerDownServerTask
{
    ...
    private final String NULL_CONNECTOR_EXCEPTION_MSG =
        "Error in Power Down Server Task because Node is [ %s ], node is %san instance of ServerNode, and IPMI Connector is [ %s ]";

    public ServiceServerNode node;

    IpmiTaskConnector connector;

    public PowerDownServerTask( ServiceHmsNode node )
    {
        this.node = (ServiceServerNode) node;
    }

    public PowerDownServerTask( ServiceHmsNode node, IpmiTaskConnector connector )
    {
        this( node );
        ...
        throws Exception, IpmiServiceResponseException
    {
        ...
        {
            logger.debug( "Received request to execute ipmi PowerDownServer task for Node " + node.getNodeID() );
            try
            {
                ...
                ChassisControlResponseData data =
                    (ChassisControlResponseData) connector.getConnector().sendMessage( connector.getHandle(),
                                                                                       chassisControl );
                logger.info( "Powered Down Node [ " + node.getNodeID() + " ]" );
                return true;
            }
            catch ( IPMIException e )
            {
                logger.error( "Exception while executing the task PowerDownServer: " + e.getCompletionCode() + ":"
                    + e.getMessage() );

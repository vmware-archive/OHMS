...
public class CumulusRouteHelper {
    ...
    /**
     * There can only be 1 default route so when being set if an existing port
     * is already used for default route then we reset the route on that port
     * and set it with the passed gateway on the new port
     *
     * @param configuration
     * @param gateway
     * @param portId
    ...
    private void execUpdateIpv4DefaultRoute(SwitchNode switchNode, String gateway, String portId) throws HmsException {
        String routeCmd = CumulusConstants.SET_DEFAULT_ROUTE_CMD
                .replaceAll("\\{password\\}", switchNode.getPassword())
                .replaceAll("\\{gateway\\}", gateway)
                .replaceAll("\\{port\\}", portId);
         ...
         * @throws HmsException
         */
    private void deleteIpv4DefaultRouteImpl(Configuration configuration, SwitchNode switchNode) throws HmsException {
        for (SwitchPort switchPort : configuration.switchPorts) {

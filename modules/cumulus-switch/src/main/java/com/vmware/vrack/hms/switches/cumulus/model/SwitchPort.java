...
/**
 * Provides basic implementation for Switch Port objects. (Inner classes for parsing cumulus Interfaces for 2.5)
 *
 * Created by sankala on 12/5/14.
 */
public class SwitchPort extends ConfigBlock {
    ...
    protected String getVlanConfig() {
        ...
        if (!this.name.equals(TOR_SWITCH_SPINE_BOND_NAME)) {
            if ( accessVlan == null || accessVlan.equals("") ) accessVlan = pvid;
    ...
    static Pattern switchPortPattern = Pattern.compile(
            String.format("\\s*(bridge-vids|bridge-pvid|bridge-access|mtu|%s|%s) (.*)$", Ipv4DefaultRoute.command,
                    Ipv4DefaultRoute.mgmtGateway));
	...
	/**
     * @param ipv4Route
     *            the ipv4Route to set
     */
    public void setIpv4DefaultRoute(Ipv4DefaultRoute ipv4Route) {
        this.ipv4DefaultRoute = ipv4Route;
    }
    
    public void deleteIpv4DefaultRoute() throws HmsException {
    	this.ipv4DefaultRoute = null;
    	Bridge bridge = this.getParentBridge();
    	
    	if (bridge == null) {
    		if (this.parentConfig != null && this.parentConfig.bridges != null && !this.parentConfig.bridges.isEmpty()) {
    			bridge = this.parentConfig.bridges.get(0);
    			this.setParentBridge(bridge);
    		}
    		else
    			throw new HmsException("Catastrophic!!! Cumulus configuration may have been damaged. No parent VLAN aware bridge found for port " + this.name);
    	}

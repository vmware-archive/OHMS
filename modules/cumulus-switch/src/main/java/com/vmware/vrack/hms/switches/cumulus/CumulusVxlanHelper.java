...
/**
 * Provides functionality for vxlan apis. This class provides functionality for vxlans such as get vxlan, matching
 * vxlans for specified vlan, create vxlan, update vxlan etc.
 */
public class CumulusVxlanHelper
{

    /**
     * Base constructor for the VxlanHelper Constructor for class Cumulus Vxlans
     * 
     * @param service
     */
    ...
    /**
     * Get vxlans for provided switch node Attempts to get vxlans (execute command to get vxlans) that maps to a
     * specific vlan name. Calls function getSwitchVxlansMatchingVlan
     * 
    ...
    /**
     * Get Switch vxlans matching vlan Currently disabled functionality. Should return the vxlans that match to the
     * provided vlan name for the switch node object.
     * 
     * @param switchNode object
    ...
    /**
     * Original (unused) get switch vxlan matching vlan Older implementation - to get the vxlans that are associated
     * with the provided vlan.
     * 
     * @param switchNode object
    ...
    private List<SwitchVxlan> ORIGINAL_getSwitchVxlansMatchingVlan( SwitchNode switchNode, String vlanName )
    {
        ...
        for ( SwitchVlan vlan : allVlans )
        {
            ...
            String command = CumulusConstants.GET_VXLAN_COMMAND.replaceAll( "\\{filename\\}", filename );
            ...
            try
            {
                ...
                {
                    Pattern p =
                        Pattern.compile( "\\s+pre-up\\s+ip\\s+link\\s+add\\s(.*)\\s+type\\s+vxlan\\s+id\\s+(.*)\\s+svcnode\\s+(.*)\\s*" );
    ...
    /**
     * Create vxlan Create vxlan for switch node based on a vxlan provided. Currently not supported.
     * 
     * @param switchNode object
    ...
    /**
     * Original Create Vxlan Older implementation - to create a vxlan based on the provided vxlan object. Determine if
     * that vxlan exists, remove and recreate vxlan.
     * 
     * @param switchNode object
        ...
        throws HmsException
    {
        ...
        logger.debug( "Creating VXLAN tunnel " + vxlanName + " associated with VLAN " + vlanName + " on switch "
            + switchNode.getSwitchId() );

        String filename = CumulusUtil.getVlanFilename( vlanName );
        String command =
            CumulusConstants.CREATE_VXLAN_COMMAND.replaceAll( "\\{password\\}",
                                                              CumulusUtil.qr( switchNode.getPassword() ) ).replaceAll( "\\{vxlan\\}",
                                                                                                                       vxlanName ).replaceAll( "\\{vni\\}",
                                                                                                                                               vxlan.getVni() ).replaceAll( "\\{vlan\\}",
                                                                                                                                                                            vlanName ).replaceAll( "\\{svcNodeIp\\}",
                                                                                                                                                                                                   CumulusConstants.SPINE_SWITCH_ANYCAST_IP ).replaceAll( "\\{filename\\}",
                                                                                                                                                                                                                                                          filename );
    ...
    /**
     * Delete Vxlan Delete vxlan for switch node based on the vxlan provided. Currently not supported.
     * 
     * @param switchNode object
    ...
    /**
     * Original implementation of Delete Older implementation - delete a vxlan based on the vxlan name provided, that is
     * associated with the provided vlan.
     * 
     * @param switchNode object
     * @param vxlanName name of vxlan to be deleted
     ...
     */
    @SuppressWarnings( "unused" )
    private boolean ORIGINAL_deleteVxlan( SwitchNode switchNode, String vxlanName, String vlanName )
        throws HmsException
    {
        /* Validate input */
        ...
        logger.debug( "Deleting VXLAN tunnel " + vxlanName + " associated with VLAN " + vlanName + " on switch "
            + switchNode.getSwitchId() );

        String filename = CumulusUtil.getVlanFilename( vlanName );
        String command =
            CumulusConstants.DELETE_VXLAN_COMMAND.replaceAll( "\\{password\\}",
                                                              CumulusUtil.qr( switchNode.getPassword() ) ).replaceAll( "\\{vxlan\\}",
                                                                                                                       vxlanName ).replaceAll( "\\{vlan\\}",

...
/**
 * All constants for Cumulus This class provides all contsants/commands and snippets of output.
 */
public class CumulusConstants
{
    ...
    public static final String CHANGE_PORT_STATUS_COMMAND =
        "echo '{password}' | sudo -S ip link set dev {portName} {status}";

    /** Reboot switch command */
    public static final String SWITCH_REBOOT_COMMAND = "echo '{password}' | sudo -S reboot";

    /** Upgrade switch command */
    public static final String SWITCH_UPGRADE_COMMAND =
        "echo '{password}' | sudo -S /bin/bash -c '" + "/usr/cumulus/bin/cl-img-install -f {package}\n"
    ...
    public static final String GET_SERIAL_ID_COMMAND =
        "echo '{password}' | sudo -S /usr/cumulus/bin/decode-syseeprom -e";

    /** Get Part Number command */
    public static final String GET_PART_NUMBER_COMMAND =
        "echo '{password}' | sudo -S /usr/cumulus/bin/decode-syseeprom | egrep ^Part | awk '{print $5}'";

    /** Get Manufacture Date command */
    public static final String GET_MANUFACTURE_DATE_COMMAND =
        "echo '{password}' | sudo -S /usr/cumulus/bin/decode-syseeprom | egrep ^\"Manufacture Date\" | tr -s ' '| cut -d' ' -f5-";

    /** Get firmware version command */
    public static final String GET_FIRMWARE_VER_COMMAND =
        "echo '{password}' | sudo -S /usr/sbin/fw_printenv onie_version | cut -d= -f2";
    ...
    public static final String PORT_GET_LOW_LEVEL_DETAILS_COMMAND =
        "echo '{password}' | sudo -S /sbin/ethtool {portName}";
    ...
    public static final String RELOAD_INTERFACES =
        "echo '{password}' | sudo -S touch /etc/network/interfaces.d/hms-port-dummy;\n"
            + "echo '{password}' | sudo -S ifreload -a";

    /** Change switch IP address command */
    public static final String CHANGE_SWITCH_IP_COMMAND = "if [ -n \"{address}\" ]; then\n"
    ...
    public static final String SHOW_LACP_IP_ADDR = "ip -br addr show {lacpGroupName} | awk  '{print $3}'";

    /** Change password command */
    public static final String CHANGE_PASSWORD_COMMAND =
        "echo '{password}' | sudo -S /bin/bash -c '/usr/sbin/chpasswd << \"EOF\" \n" + "{username}:{newPassword}\n"
            + "EOF\n'";
    ...
    public static final String SET_TIME_COMMAND =
        "val=`date -u -d @{seconds} +\"%D %T\"`; echo {password} | sudo -S date -s \"$val\"; echo {password} | sudo -S hwclock --systohc";
    ...
    public static final String CONFIGURE_BGP_COMMAND = "echo '{password}' | sudo -S /bin/bash -c \""
        + "cl-bgp as {localAsn} neighbor add {peerIp} remote-as {peerAsn};\n" + "{exportNetworksCommands}\"";

    public static final String EXPORT_NETWORK_COMMAND = "cl-bgp as {localAsn} network add {network} ipv4 unicast;\n";
    ...
    /** Set MTU command */
    public static final String SET_MTU_COMMAND = "echo '{password}' | sudo -S ip link set dev {name} mtu {mtu}";
    ...
    public static final String SET_SPEED_DUPLEX_AUTONEG_PREAMBLE = "echo '{password}' | sudo -S ethtool -s {name}";

    public static final String SET_SPEED_ADDENDUM = "speed {speed}";

    public static final String SET_DUPLEX_ADDENDUM = "duplex {duplex}";

    public static final String SET_AUTONEG_ADDENDUM = "autoneg {autoneg}";
    ...
    /** Copy command */
    public static final String COPY_AS_ROOT_COMMAND = "echo '{password}' | sudo -S cp -pf {old} {new}";
    ...
    /** Delete VLAN command */
    public static final String DELETE_VLAN_COMMAND = "echo '{password}' | sudo -S /sbin/ifdown --with-depends {vlan};\n"
        + "echo '{password}' | sudo -S rm -f {filename};";

    /** Bring up interfaces command */
    public static final String IFUP_COMMAND = "echo '{password}' | sudo -S /sbin/ifup --with-depends {interfaces}";

    /** Create VXLAN command */
    public static final String CREATE_VXLAN_COMMAND = "echo '{password}' | sudo -S sed -r -i -e 's/^(iface {vlan}.*)/"
    ...
    public static final String SETUP_PERSISTENCE_DIRECTORY = "if [ -d /mnt/persist ]; then\n"
    ...
    public static final String SET_NTP_SERVER_COMMAND = "echo '{password}' | sudo -S /bin/bash -c \" echo \\\""
    ...
    public static final String SWITCH_PORT_STANZA =
        "auto {name}\n" + "iface {name} inet {mode}\n" + "{ipv4}" + "{gateway}" + "{mtu}" + "{others}";

    /** MTU details */
    public static final String MTU_LINE = "       mtu {mtu}\n";
    ...
    /** Speed, Auto Negotation and Duplex mode details */
    public static final String SPEED_LINE_ADDENDUM = "       link-speed {speed}\n";

    /** IPV4 details */
    public static final String IPV4_LINE = "		address {address}\n" + "		netmask {netmask}\n";

    /** IPV4 Address details */
    public static final String IPV4_ADDRESS_LINE = "		address {address}\n";

    /** STP details */
    public static final String STP_LINE = "		bridge-stp on\n" + "		mstpctl-forcevers {protocol}\n";

    /** Gateway detail */
    public static final String GATEWAY_LINE = "		gateway {gateway}\n";

    /** LACP group detail */
    public static final String LACP_GROUP_STANZA =
        "auto {name}\n" + "iface {name}\n" + "{ipv4}" + "{gateway}" + "		bond-slaves {interfaces}\n"
            + "		bond-mode {lacpMode}\n" + "		bond-miimon 100\n" + "		bond-use-carrier 1\n"
    ...
    /** VLAN detail */
    public static final String VLAN_STANZA = "auto {name}\n" + "iface {name} inet {mode}\n"
    ...
    public static final String CLAG_STANZA =
        "auto {name}\n" + "iface {name}\n" + "{ipv4}" + "	clagd-enable {enabled}\n" + "	clagd-priority 8192\n"
            + "	clagd-peer-ip {peerIp}\n" + "	clagd-sys-mac {sharedMac}\n";

    /** VLAN CL25 detail */
    public static final String VLAN_CL25_STANZA = "auto {name}\n" + "iface {name}\n" + "	bridge-vlan-aware yes\n"
        + "	bridge-ports {interfaces}\n" + "	bridge-vids {id}\n" + "	bridge-pvid 1\n" + "	bridge-stp on\n";

/* ********************************************************************************
 * CumulusEtcNetworkInterfaces.java
 * 
...
/**
 * Parses the Network Configuration for Cumulus Switch. This class serves to parse and pull together the Cumulus network
 * interface details.
 */
@Deprecated
public class CumulusEtcNetworkInterfaces
{
    ...
    /**
     * CumulusEtcNetworkInterfaces empty constructor. Empty constructor for the CumulusEtcNetworkInterface class.
     */
    ...
    /**
     * Parses the network configuration details for the cumulus switch instance. Utilizing the
     * SwitchNetworkConfiguration object from the switch instance: 1. parses the details 2. structures the output using
     * the CumulusConstants with the network interface details.
     * 
    ...
    public static CumulusEtcNetworkInterfaces parseNetworkConfiguration( SwitchNetworkConfiguration networkConfiguration )
    {
        ...
        ret.contents.add( CumulusConstants.HMS_SIGNATURE_STANZA.replaceAll( "\\{hms.configuration.name\\}",
                                                                            networkConfiguration.getName() ).replaceAll( "\\{hms.configuration.version\\}",
                                                                                                                         networkConfiguration.getVersion() ).replaceAll( "\\{hms.configuration.applied\\}",
                                                                                                                                                                         new Date( System.currentTimeMillis() ).toString() ).replaceAll( "\\{hms.configuration.signature\\}",
        ...
        ret.contents.add( CumulusConstants.MANAGEMENT_PORT_STANZA.replaceAll( "\\{ipv4\\}",
                                                                              CumulusConstants.IPV4_LINE ).replaceAll( "\\{gateway\\}",
                                                                                                                       CumulusConstants.GATEWAY_LINE ).replaceAll( "\\{address\\}",
                                                                                                                                                                   networkConfiguration.getManagement().getIpv4().getAddress() ).replaceAll( "\\{netmask\\}",
                                                                                                                                                                                                                                             networkConfiguration.getManagement().getIpv4().getNetmask() ).replaceAll( "\\{gateway\\}",
                                                                                                                                                                                                                                                                                                                       networkConfiguration.getManagement().getIpv4().getGateway() ) );
            ...
            for ( Port p : portList )
            {
                ...
                if ( p.getMtu() != null )
                {
                    ...
                    mtu = CumulusConstants.MTU_LINE.replaceAll( "\\{mtu\\}",
                                                                p.getMtu().toString() ).replaceAll( "\\{name\\}",
                                                                                                    p.getName() );
                }
                ...
                if ( portIpv4Map.containsKey( p.getName() ) )
                {
                    ...
                    address =
                        CumulusConstants.IPV4_LINE.replaceAll( "\\{address\\}",
                                                               ipv4.getAddress() ).replaceAll( "\\{netmask\\}",
                                                                                               ipv4.getNetmask() );
                }

                if ( writeStanza )
                {
                    ret.contents.add( CumulusConstants.SWITCH_PORT_STANZA.replaceAll( "\\{name\\}",
                                                                                      p.getName() ).replaceAll( "\\{mode\\}",
        ...
        for ( String lg : lacpGroupPortMap.keySet() )
        {
            ...
            if ( portIpv4Map.containsKey( lg ) )
            {
                ...
                mode = "static";
                address = CumulusConstants.IPV4_LINE.replaceAll( "\\{address\\}",
                                                                 ipv4.getAddress().replaceAll( "\\{netmask\\}",
                                                                                               ipv4.getNetmask() ) );
            }

            ret.contents.add( CumulusConstants.LACP_GROUP_STANZA.replaceAll( "\\{name\\}",
                                                                             lg ).replaceAll( "\\{interfaces\\}",
                                                                                              interfaceListStr.toString() ).replaceAll( "\\{mode\\}",
                                                                                                                                        mode ).replaceAll( "\\{lacpMode\\}",
                                                                                                                                                           "802.3ad" ).replaceAll( "\\{ipv4\\}",
                                                                                                                                                                                   address ).replaceAll( "\\{gateway\\}",
                                                                                                                                                                                                         gateway ) );
        }
        ...
        for ( Vlan v : networkConfiguration.getVlans() )
        {
            ...
            {
                ret.contents.add( CumulusConstants.VLAN_SUBINTERFACE_STANZA.replaceAll( "\\{id\\}",
                                                                                        Integer.toString( v.getVlanId() ) ).replaceAll( "\\{subifname\\}",
                                                                                                                                        port ) );
            ...
            if ( portIpv4Map.containsKey( v.getName() ) )
            {
                ...
                ipv4Str.append( CumulusConstants.IPV4_LINE.replaceAll( "\\{address\\}",
                                                                       ipv4.getAddress() ).replaceAll( "\\{netmask\\}",
                                                                                                       ipv4.getNetmask() ) );
            }
            ...
            {
                stpStr.append( CumulusConstants.STP_LINE.replaceAll( "\\{protocol\\}", v.getStp().getProtocol() ) );
            }
            ...
            if ( networkConfiguration.getRoutes() != null )
            {
                for ( Vrr vrr : networkConfiguration.getRoutes().getVrr() )
                {
                    ...
                    {
                        vrrStr.append( CumulusConstants.VRR_LINE.replaceAll( "\\{controlInterface\\}",
                                                                             vrr.getControlInterface() ).replaceAll( "\\{sharedMac\\}",
                                                                                                                     vrr.getSharedMac() ).replaceAll( "\\{sharedIp\\}",
                                                                                                                                                      vrr.getSharedIp() ) );
                        break;
                    }
                }
            ...
            ret.contents.add( CumulusConstants.VLAN_STANZA.replaceAll( "\\{ipv4\\}",
                                                                       ipv4Str.toString() ).replaceAll( "\\{stp\\}",
                                                                                                        stpStr.toString() ).replaceAll( "\\{vrr\\}",
                                                                                                                                        vrrStr.toString() ).replaceAll( "\\{name\\}",
                                                                                                                                                                        v.getName() ).replaceAll( "\\{interfaces\\}",
                                                                                                                                                                                                  interfaceListStr.toString() ) );
        }
        ...
        if ( networkConfiguration.getRoutes() != null )
        {
            ...
            {
                ret.contents.add( CumulusConstants.CLAG_STANZA.replaceAll( "\\{name\\}",
                                                                           c.getInterface() ).replaceAll( "\\{peerIp\\}",
                                                                                                          c.getPeerIp() ).replaceAll( "\\{sharedMac\\}",
                                                                                                                                      c.getSharedMac() ) );
            }
        }

...
public class Svi extends ConfigBlock {
    ...
    static Pattern sviPattern = Pattern.compile(String.format("\\s*(%s|%s) (.*)$", addressLiteral, querierIpLiteral));

    /*
     * We only have what we need now and nothing else, hence we dont have ports here which is managed purely using
     * VLAN aware bridge configuration as of now
     */
    ...
    public String getString() {
        ...
        if ( address != "" && prefixLen != 0) retString += "\n    " + addressLiteral + " " + getAddress() + "/" + getPrefixLen();
        if ( igmpQuerierAddress != null && igmpQuerierAddress != "") retString += "\n    " + querierIpLiteral + " " + getIgmpQuerierAddress();

...
/**
 * Provides basic implementation for the object Bridge. Extends Switch Port implementation. Created by sankala on
 * 12/5/14.
 */
public class Bridge
    extends ConfigBlock
{
    ...
    static Pattern bridgePattern =
        Pattern.compile( "\\s*(bridge-vlan-aware|bridge-ports|bridge-vids|bridge-pvid|bridge-access) (.*)$" );
     ...
     * @return
     */
    public SwitchPort getMemberByName( String name )
    {
        for ( SwitchPort switchPort : members )
        {
            if ( switchPort.name.equals( name ) )
                return switchPort;
        }

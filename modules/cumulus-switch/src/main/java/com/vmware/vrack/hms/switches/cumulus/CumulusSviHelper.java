...
public class CumulusSviHelper
{
    ...
    /**
     * Function to retrieve SVI configurations from configuration and put them inside corresponding VLANs already
     * present inside vlansMap. if the entry is not present inside the map then we throw Exception.
     *
     * @param vlansMap
     * @param configuration
     */
    public static void getVlansFromConfigurarationIntoMap( Map<String, SwitchVlan> vlansMap,
                                                           Configuration configuration )
        throws HmsOobNetworkException
    {

        if ( configuration == null )
            throw new HmsOobNetworkException( "Configuration cannot be NULL",
        ...
        if ( configuration.svis != null && !configuration.svis.isEmpty() )
        {
            for ( Svi svi : configuration.svis )
            {
                ...
                if ( vlan == null )
                {
                    throw new HmsOobNetworkException( "The VLAN Map must contain the VLANs for which SVI exists in Cumulus configuration",
    ...
    /**
     * Function to update Cumulus specific persistent configuration inside configuration object from vlan mainly values
     * related to IP address, IGMP support etc.
     *
     * @param vlan
     * @param configuration
     */
    public static void updateVlanInConfiguration( String vlanAwareBridgeName, SwitchVlan vlan,
                                                  Configuration configuration )
        throws HmsOobNetworkException
    {
        ...
        if ( vlan.getIpAddress() == null )
            return; // do nothing since we act on an existing or a ne SVI if and only if an IP address is specified
        ...
        for ( Svi svi : configuration.svis )
        {
            if ( svi.pvid.equals( vlan.getId() ) )
            {
                ...
                {
                    svi.setIgmpQuerierAddress( vlan.getIpAddress() ); /*
                                                                       * IGMP uses the IP address configured in the VLAN
                                                                       */
                }
                else
                {
     ...
     * @param configuration
     */
    public static void deleteVlanInConfiguration( SwitchVlan vlan, Configuration configuration )
        throws HmsOobNetworkException
    {

...
/**
 * Encompasses Cumulus Switch Service utility functions.
 * 
 * This class provides functionality for the switch service utility functions . 
 */
public class CumulusUtil {
	
	/**
	 * Return SSH session to switchNode.
	 * 
	 * With provided switch node object, get the session and connect. 
	 * 
	...
	/**
	 * Upload a file as root user.
	 * 
	 * The SSH connection is made as cumulus user, but this does not give us sufficient permissions 
	 * to create files in /etc/network/interfaces.d directory, for example. So, we need to
	 * upload to a temporary directory and then copy to final destination.
	 * 
	 * @param switchNode object
	 ...
	 */
	public static void uploadAsRoot(SwitchNode switchNode, ByteArrayInputStream bais, String remoteFilename) throws HmsException {
		CumulusTorSwitchSession session = (CumulusTorSwitchSession) getSession(switchNode);
		String temporaryFilename = CumulusConstants.PERSISTENT_TMP_DIR + "/" + remoteFilename.replaceAll("/", "_").replaceAll("^\\.*", "");

		/* Upload the file to a temporary location where we have permission */
		boolean uploaded = session.upload(bais, temporaryFilename);
		
		if (!uploaded) {
			throw new HmsException ("Failed to upload file " + remoteFilename + " to switch " + switchNode.getSwitchId());
		}

		/* Copy the file to its final destination. */
		String copyCommand = CumulusConstants.COPY_AS_ROOT_COMMAND
				.replaceAll("\\{password\\}", qr(switchNode.getPassword()))
				.replaceAll("\\{old\\}", temporaryFilename)
				.replaceAll("\\{new\\}", remoteFilename);

		SshExecResult result = session.executeEnhanced(copyCommand);
		result.logIfError(logger);
		if (result.getExitCode() != 0) {
			throw new HmsException ("Failed to copy uploaded file " + remoteFilename + " on switch " + switchNode.getSwitchId());
		}
	}
	 ...
	 * 
	 * Persist all the relevant configuration files to be retained across reboots/upgrades.
	 * 
	 * @param switchNode object
	 * @return void
	 * @throws HmsException if Setup of persistent director fails
	 */
	public static void configurePersistenceDirectory(SwitchNode switchNode) throws HmsException {
		String password = switchNode.getPassword();
		String command = CumulusConstants.SETUP_PERSISTENCE_DIRECTORY
				.replaceAll("\\{password\\}", qr(password));
		CumulusTorSwitchSession session = (CumulusTorSwitchSession) getSession(switchNode);
	 ...
	 * 
	 * Validate that the base interfaces file contains the source clause. If necessary, add it.
	 * 
	 * @param switchNode object
	 * @return void
	 * @throws HmsException thrown if there is a failure to add source clause to the interfaces on the switch
	 */
	public static void validateSourceClause(SwitchNode switchNode) throws HmsException {
		CumulusTorSwitchSession session = (CumulusTorSwitchSession) getSession(switchNode);
		String command = CumulusConstants.ADD_SOURCE_CLAUSE_COMMAND
				.replaceAll("\\{password\\}", qr(switchNode.getPassword()));
		
		SshExecResult result = session.executeEnhanced(command);
		result.logIfError(logger);
		if (result.getExitCode() != 0) {
			throw new HmsException ("Failed to add source clause to /etc/network/interfaces on switch " + switchNode.getSwitchId());
		}
	}
	 ...
	 * getPortFileName
	 * 
	 * Get port filename - helper function. 
	 * 
	...
	/**
	 * getLacpGroupFilename
	 * 
	 * Get lacp group filename
	 * 
	 ...
	 * 
	 * Helper function to return literal replacement for the provided string. 
	 * 

...
/**
 * Implements BGP (Border Gateway Protocol) - a routing protocol. This class serves as the base BGP for a cumulus
 * switch, and will be used to configure, enable or disable bgp.
 */
public class CumulusBgpHelper
{
    /**
     * Bgp Helper class. Sets the Bgp service provided for the CumulusTorSwitchService object created.
     *
    ...
    /**
     * Configure BGP (enable or disable). Enable or Disable BGP configuration for the cumulus switch session.
     *
    ...
    /**
     * Get the BGP configuration/protocol. Get the BGP configuration for the cumulus switch session.
     *
     * @param switchNode information about the switch (including ip, username/password, protocol etc).
     */
    ...
    /**
     * Enable BGP. Enable BGP configuration for the cumulus switch session.
     *
     * @param switchNode information about the switch (including ip, username/password, protocol etc).
     * @param bgp BGP configuration
     * @exception HmsException if the CumulusTorSwitchSession failed to enable the BGP on the switch.
     */
    private boolean enableBgp( SwitchNode switchNode, SwitchBgpConfig bgp )
        throws HmsException
    {
        ...
        String startDaemonCommand =
            CumulusConstants.ENABLE_BGP_COMMAND.replaceAll( "\\{password\\}",
        ...
        {
            String command =
                CumulusConstants.EXPORT_NETWORK_COMMAND.replaceAll( "\\{network\\}",
                                                                    network ).replaceAll( "\\{localAsn\\}",
                                                                                          Integer.toString( bgp.getLocalAsn() ) );

            exportNetworksCommands.append( command );
        }

        String configureBgpCommand =
            CumulusConstants.CONFIGURE_BGP_COMMAND.replaceAll( "\\{password\\}",
                                                               CumulusUtil.qr( switchNode.getPassword() ) ).replaceAll( "\\{localAsn\\}",
                                                                                                                        Integer.toString( bgp.getLocalAsn() ) ).replaceAll( "\\{peerAsn\\}",
                                                                                                                                                                            Integer.toString( bgp.getPeerAsn() ) ).replaceAll( "\\{peerIp\\}",
                                                                                                                                                                                                                               bgp.getPeerIpAddress() ).replaceAll( "\\{exportNetworksCommands\\}",
                                                                                                                                                                                                                                                                    exportNetworksCommands.toString() );
    ...
    /**
     * Disable BGP. Disable BGP configuration for the cumulus switch session.
     *
     * @param switchNode information about the switch (including ip, username/password, protocol etc).
     * @param bgp BGP configuration
     * @exception HmsException if CumulusTorSwitchSession failed to disable BGP on the switch
     */
    private boolean disableBgp( SwitchNode switchNode, SwitchBgpConfig bgp )
        throws HmsException
    {
        /* Stop BGP daemon via Quagga */
        String startDaemonCommand =
            CumulusConstants.DISABLE_BGP_COMMAND.replaceAll( "\\{password\\}",
                                                             CumulusUtil.qr( switchNode.getPassword() ) );

...
import com.vmware.vrack.hms.common.switches.api.SwitchSnmpUser.AuthType;
import com.vmware.vrack.hms.common.switches.api.SwitchSnmpUser.PrivType;
...
/**
 * Provides functionality for the entire Cumulus Switch Service. This class provides functionality for the switch
 * service. Acts as a base class, all switch classes extend this class.
 */
@SwitchServiceImplementation( name = CumulusConstants.CUMULUS_SWITCH_TYPE )
public class CumulusTorSwitchService
    implements ISwitchService
{
    ...
    /**
     * Discovers switch for provided switch node Checks protocol (SSH), and executes command to discover the existing
     * cumulus switch.
     *
    ...
    /**
     * Determine if switch is powered on For provided switch node object, get the ipAddress and determine if the switch
     * is reachable based on the timeout variable.
     * 
     * @param switchNode switch node object
    ...
    /**
     * Gets the Switch's os information For provided switch node, get the the switch operating system details, get last
     * boot time, and firmware name and version.
     * 
     * @param switchNode switch node object
     * @return switch os info object with all details about the os.
     * @exception e Thrown if OS details failed to be gathered, or exception raised during last boot, or if Firmware
     *                details failed to be gathered
     */
    @Override
    public SwitchOsInfo getSwitchOsInfo( SwitchNode switchNode )
    {
        ...
        try
        {
            String firmwareVersionCommand =
                CumulusConstants.GET_FIRMWARE_VER_COMMAND.replaceAll( "\\{password\\}",
                                                                      qr( switchNode.getPassword() ) );
    ...
    /**
     * Get the switch hardware info Using the switch node, get the hardware model, the serial ID and the management port
     * name.
     * 
     * @param switchNode switch node object
    ...
    public SwitchHardwareInfo getSwitchHardwareInfo( SwitchNode switchNode )
    {
        ...
        try
        {
            SshExecResult result =
                switchSession.executeEnhanced( CumulusConstants.HARDWARE_MODEL_MANUFACTURER_COMMAND );
            result.logIfError( logger );
            if ( result.getExitCode() != 0 )
                cacheResult = false;
            ...
            String serialCommand =
                CumulusConstants.GET_SERIAL_ID_COMMAND.replaceAll( "\\{password\\}", qr( switchNode.getPassword() ) );
            ...
            String partNumberCommand =
                CumulusConstants.GET_PART_NUMBER_COMMAND.replaceAll( "\\{password\\}", qr( switchNode.getPassword() ) );
            ...
            String manufactureDateCommand =
                CumulusConstants.GET_MANUFACTURE_DATE_COMMAND.replaceAll( "\\{password\\}",
                                                                          qr( switchNode.getPassword() ) );
            ...
            {
                logger.warn( "Couldn't determine management MAC address of management port on switch "
        ...
        {
            logger.warn( "Exception received while getting hardware model and serial number on switch "
                + switchNode.getSwitchId(), e );
            cacheResult = false;
        }
    ...
    /**
     * Update the switch IP address with new details For the given switch node, execute command to change the IP
     * address, netmask, and gateway. Configure persistence directory and make changes effective immediately.
     * 
     * @param switchNode switch node object
        ...
        throws HmsException
    {
        ...
        String command =
            CumulusConstants.CHANGE_SWITCH_IP_COMMAND.replaceAll( "\\{password\\}",
                                                                  qr( switchNode.getPassword() ) ).replaceAll( "\\{address\\}",
                                                                                                               ipAddress != null
                                                                                                                               ? ipAddress.trim()
                                                                                                                               : "" ).replaceAll( "\\{netmask\\}",
                                                                                                                                                  netmask != null ? netmask.trim()
                                                                                                                                                                  : "" ).replaceAll( "\\{gateway\\}",
                                                                                                                                                                                     gateway != null ? gateway.trim()
                                                                                                                                                                                                     : "" );
        ...
        command =
            CumulusConstants.IFUP_COMMAND.replaceAll( "\\{password\\}",
                                                      qr( switchNode.getPassword() ) ).replaceAll( "\\{interfaces\\}",
                                                                                                   CumulusConstants.MANAGEMENT_PORT_NAME );
    ...
    /**
     * Update the switch password with provided one For the given switch node, execute command to change password of
     * specific user with provided password
     *
     * @param switchNode switch node object
     * @param username username the password we change
     * @param newPassword updated password
     * @return True if update was successful; False if update was unsuccessful
     * @exception HmsException if error in rotating the password
     */
    @Override
        ...
        throws HmsException
    {
        ...
        String command =
            CumulusConstants.CHANGE_PASSWORD_COMMAND.replaceAll( "\\{password\\}",
                                                                 qr( switchNode.getPassword() ) ).replaceAll( "\\{username\\}",
                                                                                                              username ).replaceAll( "\\{newPassword\\}",
                                                                                                                                     newPassword );
        ...
        if ( result.getExitCode() != 0 )
        {
            throw new HmsException( "Error rotating the password of switch " + switchNode.getSwitchId() );
        }

        switchNode.setPassword( newPassword ); // update in-memory object
        ...
        // ToDo: update local hms inventory with new password
        ...
        throws HmsException
    {
        ...
        String command =
            CumulusConstants.SET_NTP_SERVER_COMMAND.replaceAll( "\\{password\\}",
                                                                qr( switchNode.getPassword() ) ).replaceAll( "\\{timeServer\\}",
                                                                                                             timeServer.trim() );
    ...
    /**
     * Get the switch port list For the switch node, execute command to get list of ports.
     * 
     * @param switchNode switch node object
    ...
    /**
     * Get switch port object For the switch node, using the port name - get the switch port
     * 
     * @param switchNode switch node object
    ...
    /**
     * Get the switch port status Using the switch node and node name, get the status of the switch port.
     * 
     * @param switchNode switch node object
    ...
    /**
     * Set the switch port status For the switch node, set the switch port status with the new portStatus value.
     * 
     * @param switchNode switch node object
        ...
        throws HmsException
    {
        ...
        success = portStatusHelper.setSwitchPortStatusRuntimeOnly( switchNode, portName, portStatus );

        /*
         * This is the hack to save an empty port inside configuration file if no port is there. This is the current
         * behaviour as on 5th April 2016 where VIA does not create any stanza for the uplink ports and later through
         * HMS, VRM enables the uplink ports and then we need to add stanza for them
         */
        if ( portStatus.equals( SwitchPort.PortStatus.UP ) )
            success &= portStatusHelper.createNewPortStanzaIfAbsent( switchNode, portName );
        ...
        throws HmsOobNetworkException
    {
        ...
        if ( newPort.getStatus() != null )
        {
            try
            {
                portStatusHelper.setSwitchPortStatusRuntimeOnly( switchNode, portName, newPort.getStatus() );
            }
        ...
        if ( newPort.getMtu() > 0 )
        {
            ...
            String setMtuCommand =
                CumulusConstants.SET_MTU_COMMAND.replaceAll( "\\{password\\}",
                                                             qr( switchNode.getPassword() ) ).replaceAll( "\\{name\\}",
                                                                                                          portName ).replaceAll( "\\{mtu\\}",
                                                                                                                                 Integer.toString( newPort.getMtu() ) );
            ...
            writeStanza = true;
            mtu = CumulusConstants.MTU_LINE.replaceAll( "\\{mtu\\}",
                                                        Integer.toString( newPort.getMtu() ) ).replaceAll( "\\{name\\}",
                                                                                                           portName );
        }

        /*
         * Invoke ethtool to modify 'all' of the three parameters - speed, duplex and autoneg. If speed is specified as
         * "default", do not pass the speed parameter to ethtool and there should be no speed entry for the port in
         * interfaces file. We should ideally also set the port to the speed specified in /etc/cumulus/ports.conf, but
        ...
        if ( ( newPort.getSpeed() != null ) && ( newPort.getAutoneg() != null ) && ( newPort.getDuplex() != null ) )
        {
            ...
            {
                error( "Unsupported port speed. It can have values 1G, 10G, 40G only",
            ...
            {
                error( "Unsupported duplex mode. It can be FULL or HALF only",
                       HmsOobNetworkErrorCode.ARGUMENT_SYNTAX_ERROR );
            }
            ...
            {
                error( "Unsupported autoneg mode. It can be ON or OFF only",
                       HmsOobNetworkErrorCode.ARGUMENT_SYNTAX_ERROR );
            }

            String setCommand =
                CumulusConstants.SET_SPEED_DUPLEX_AUTONEG_PREAMBLE.replaceAll( "\\{password\\}",
                                                                               qr( switchNode.getPassword() ) ).replaceAll( "\\{name\\}",
                                                                                                                            portName );
            ...
            if ( speedInMBPerSec > 0 )
                setCommand +=
                    " " + CumulusConstants.SET_SPEED_ADDENDUM.replaceAll( "\\{speed\\}",
                                                                          Integer.toString( speedInMBPerSec ) );

            setCommand += " " + CumulusConstants.SET_DUPLEX_ADDENDUM.replaceAll( "\\{duplex\\}", duplex );

            setCommand += " " + CumulusConstants.SET_AUTONEG_ADDENDUM.replaceAll( "\\{autoneg\\}", autoNeg );
            ...
            {
                error( "Failed to set Speed = " + newPort.getSpeed() + ", Auto Negotation Mode = " + autoNeg
                    + ", Duplex Mode = " + duplex + " on switch port " + portName
                    + " (Reason could not be retrieved. Is Null)", HmsOobNetworkErrorCode.SET_OPERATION_FAILED );
            }
            if ( result != null && result.getExitCode() != 0 )
            {
                if ( result.getStdout().length > 0 )
                ...
                error( "Failed to set Speed = " + newPort.getSpeed() + ", Auto Negotation Mode = " + autoNeg
                    + ", Duplex Mode = " + duplex + " on switch port " + portName + ". Reason: "
                    + result.getStdoutAsString(), HmsOobNetworkErrorCode.SET_OPERATION_FAILED );
            }

            /* Change the persistent settings in the interfaces file */
            writeStanza = true;
            others =
                CumulusConstants.AUTONEG_DUPLEX_LINES.replaceAll( "\\{name\\}",
                                                                  portName ).replaceAll( "\\{duplex\\}",
                                                                                         duplex ).replaceAll( "\\{autoneg\\}",
                                                                                                              autoNeg );

            if ( speedInMBPerSec > 0 )
                others += CumulusConstants.SPEED_LINE_ADDENDUM.replaceAll( "\\{speed\\}",
                                                                           Integer.toString( speedInMBPerSec ) );
        ...
        /*
         * Change/set/reset IP address only inside interfaces configuration file and at the end to an ifreload which
        ...
        if ( writeStanza )
        {
            ...
            String stanza =
                CumulusConstants.SWITCH_PORT_STANZA.replaceAll( "\\{name\\}",
                                                                portName ).replaceAll( "\\{mode\\}",
                                                                                       mode ).replaceAll( "\\{ipv4\\}",
                                                                                                          address ).replaceAll( "\\{gateway\\}",
                                                                                                                                gateway ).replaceAll( "\\{mtu\\}",
                                                                                                                                                      mtu ).replaceAll( "\\{others\\}",
                                                                                                                                                                        others );
            ...
            /* Upload the file */
            /*
             * All these operations are for now translated but in future these methods themselves will throw the
            ...
            {
                error( "Failed to save new configurations inside persistent directory", e,
                       HmsOobNetworkErrorCode.INTERNAL_ERROR );
            }
    ...
    /**
     * Update the switch port details For the switch node object, update details for either(all) items listed in the
     * portInfo object. Updates cover status of port, MTU, and IP address. Updates for each type is captured in the
     * portInfo object. This basically overwrites all existing configuration for this port with the new ones passed
     *
     * @param switchNode switch node object
        ...
        throws HmsException
    {
        ...
        {
            /*
             * It makes sense to retry with older configuration if and only if setting the new parameters has failed,
             * for other errors the revert process may (very likely) also fail
             */
            if ( e.getErrorCode() == HmsOobNetworkErrorCode.SET_OPERATION_FAILED )
            {
                ...
                {
                    error( "Unable to revert to previous snapshot on failure to set port properties", e1,
                           HmsOobNetworkErrorCode.ROLLBACK_FAILED );
                }
            }
            /*
             * Even if we set the earlier properties correctly we still fail the operation and rethrow the exception
             */
            throw e;
        }
    ...
    /**
     * Get vlan for specified switch node. (Implementation in CumulusVlanHelper file) Using vlanHelper class, get
     * appropriate switchVlan for switchNode
     *
     * @param switchNode switch node object
    ...
    /**
     * Get vlans for specified switch node (Implementation in CumulusVlanHelper file) Get list of Vlans for the provided
     * switchNode.
     *
     * @param switchNode switch node object
    ...
    /**
     * Get vlan in bulk format (Implementation in CumulusVlanHelper file) Get vlan details for provided switchNode in
     * bulk format.
     *
     * @param switchNode switch node object
    ...
    /**
     * Create a vlan for a switch node with the provide vlan. (Implementation in CumulusVlanHelper file) Create Vlan
     * with provided vlan object on the switchNode
     *
     * @param switchNode switch node object
    ...
    /**
     * Update vlan details Updates to the vlan object for the switchNode object (Implementation in CumulusVlanHelper
     * file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Delete vlan for provided switch node Deletion of a vlan on the provided switch node
     *
     * @param switchNode switch node object
    ...
    /**
     * Get lacp groups Get lacp groups for provided switch node (Implementation in CumulusLacpGroupHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Get Switch Lacp Group Get a lacp group for provided switch node and lacp group name (Implementation in
     * CumulusLacpGroupHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Create Lacp Group Create lacp group for the switch node (Implementation in CumulusLacpGroupHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Delete a lacp group Delete lacp group for provided switch node (Implementation in CumulusLacpGroupHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Reboot the switch node object that is provided Execute command to reboot the switch session.
     * 
        ...
        throws HmsException
    {
        ...
        boolean success = false;
        String command =
            CumulusConstants.SWITCH_REBOOT_COMMAND.replaceAll( "\\{password\\}", qr( switchNode.getPassword() ) );
    ...
    /**
     * Upgrade the switch node For passed in upgrade info, validate HTTP and FTP links first before executing upgrade
     * command to install the new image on the switch. Reboot the node so that the new upgraded details will take
     * effect.
     *
        ...
        throws HmsException
    {
        ...
        if ( lcurl.startsWith( "http://" ) || lcurl.startsWith( "ftp://" ) )
        {
            String validateCommand =
                CumulusConstants.SWITCH_UPGRADE_CHECK_PKG_COMMAND.replaceAll( "\\{package\\}",
                                                                              upgradeInfo.getPackageUrl() );
            ...
            if ( result.getExitCode() != 0 )
            {
                throw new HmsObjectNotFoundException( "Cannot upgrade because package URL is either invalid or inaccessible: "
                    + upgradeInfo.getPackageUrl() );
            }
        }
        ...
        String command =
            CumulusConstants.SWITCH_UPGRADE_COMMAND.replaceAll( "\\{password\\}",
                                                                qr( switchNode.getPassword() ) ).replaceAll( "\\{package\\}",
                                                                                                             upgradeInfo.getPackageUrl() );
        ...
        {
            throw new HmsException( "Error received while installing new OS image on switch "
                + switchNode.getSwitchId(), e );
        }
    ...
    /**
     * Apply network configuration updates to the switch node For existing switch node, apply command to update the
     * network configuration details and upload interfaces file.
     *
     * @param switchNode switch node object
     * @param networkConfiguration network configuration details
     * @return True if network configuration applied successfully; False if network configuration is not applied
     *         successfully
        ...
        throws HmsException
    {
        ...
        String command =
            CumulusConstants.APPLY_NETWORK_CFG_COMMAND.replaceAll( "\\{file\\}",
                                                                   qr( remoteTmpFile ) ).replaceAll( "\\{password\\}",
                                                                                                     qr( switchNode.getPassword() ) );
    ...
    /**
     * Get Switch Vxlans Get vxlan details details for switch node (Implementation in CumulusVxlanHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Get Switch vxlans matching vlan Get vxlans associated with a vlan (Implementation in CumulusVxlanHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Create Vxlan Create new vxlan for switch node (Implementation in CumulusVxlanHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Delete Vxlan Delete vxlan for switch node (Implementation in CumulusVxlanHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Delete a port or a bond from existing VLAN
     */
    @Override
        ...
        throws HmsOobNetworkException
    {
        ...
        vlanHelper.deletePortFromVlanConfiguration( switchNode, vlanId, portOrBondName );
    }

    /**
     * Delete port from existing LACP Group
     */
    @Override
        ...
        throws HmsOobNetworkException
    {
        ...
        lacpHelper.deleteSwitchPortFromLacpGroup( switchNode, lacpGroupName, portName );
    }

    /**
     * Get switch port list (bulk format) Using the switch node provided, get the switch port list (list of switch
     * ports). Gather the lldp, fdb, and port results. Parse for MTU, Mac Address, link speed, RX and TX packet counts,
     * add to specific port and return list of switch ports.
     *
     * @param switchNode switch node object
    ...
    public List<SwitchPort> getSwitchPortListBulk( SwitchNode switchNode )
    {
        ...
        String[] commandSet = {
            ...
            "/bin/bridge fdb show", "ip -s link show", "ip -br addr show",
            /*
             * Get per port details, due to limitation of 'ethtool' command which works only with a port name and not on
             * a list of interfaces/wildcard. Hence, this is the only way to preserve efficiency of this method and yet
             * achieve end goal
             */
            ( "for port in `ls -l /sys/class/net/ | awk '{if (/.*swp.*/) print $9; else if (/.*eth.*/) print $9}'`; do echo '{password}' | sudo -S ethtool $port; done" ).replaceAll( "\\{password\\}",
                                                                                                                                                                                      switchNode.getPassword() ) };
        ...
        if ( lldpResultLines != null && lldpResultLines.length > 0 )
        {
            ...
            for ( String lldpLine : lldpResultLines )
            {
                ...
                else if ( lldpLine.matches( ".*PortID:.*" ) || lldpLine.matches( ".*ChassisID:.*" ) )
                {
                    ...
                    /*
                     * Cumulus has this weird way of prepending "iface" to the actual port name. Let's make sure we get
                     * the actual port id
                     */
        ...
        /*
         * Parse through physical properties of each port Example: Settings for swp8: Supported ports: [ ] Supported
         * link modes: Not reported Supported pause frame use: No Supports auto-negotiation: No Advertised link modes:
         * Not reported Advertised pause frame use: No Advertised auto-negotiation: No Speed: Unknown! Duplex: Half
         * Port: Twisted Pair PHYAD: 0 Transceiver: internal Auto-negotiation: off MDI-X: Unknown Current message level:
         * 0x00000000 (0) Link detected: no
         */
         ...
         */
        for ( blockStart = 0, blockEnd = blockStart + 6; blockEnd < portResultArr.length; blockStart += 6, blockEnd +=
            6 )
        {
    ...
    /**
     * Configure the OSPF Using OSPF details, configure the OSPF daemons. Execute command to enable or disable the
     * quagga daemons.
     *
     * @param switchNode switch node object
        ...
        throws HmsException
    {
        ...
        String command = ospf.isEnabled()
                        ? CumulusConstants.ENABLE_QUAGGA_DAEMONS_COMMAND.replaceAll( "\\{password\\}",
                                                                                     qr( switchNode.getPassword() ) )
                        : CumulusConstants.DISABLE_QUAGGA_DAEMONS_COMMAND.replaceAll( "\\{password\\}",
                                                                                      qr( switchNode.getPassword() ) );
    ...
    /**
     * Get the OSPF details For provided switchnode, pull bulk data from the switch and parse for router id, mode,
     * network details, interface names and details.
     *
     * @param switchNode switch node object
        ...
        throws HmsException
    {
         ...
         */
        String[] commandSet = {
            "echo '{password}' | sudo -S cat /etc/quagga/Quagga.conf".replaceAll( "\\{password\\}",
                                                                                  switchNode.getPassword() ),
        ...
        if ( ( quaggaConfig.length == 0 ) || quaggaConfig[0].matches( ".*No such file or directory.*" ) )
        {
    ...
    /**
     * Configure Bgp Configure Bgp object with the switch bgp config details. (Implementation in CumulusLacpGroupHelper
     * file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Get Bgp Configuration Get the Bgp configuration for specified switch node (Implementation in
     * CumulusLacpGroupHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Get Switch Sensor Info Get switch sensor information for specified switch node (Implementation in
     * CumulusSensorHelper file)
     *
     * @param switchNode switch node object
    ...
    /**
     * Configure Mclag details for provided switch node For the provided switch node and mclag details, update the
     * current configuration with the new IPv4 address lines. Create, upload, and start the mclag interfaces to enable
     * and configure on the switch node.
     *
     * @param switchNode switch node object
        ...
        throws HmsException
    {
        ...
        if ( mclag.isEnabled() )
        {
            ...
            if ( mclag.getIpAddress() != null )
            {
                ...
                {
                    ipv4 =
                        CumulusConstants.IPV4_LINE.replaceAll( "\\{address\\}",
                                                               mclag.getIpAddress() ).replaceAll( "\\{netmask\\}",
                                                                                                  mclag.getNetmask() );
                }
                else
                {
                    ipv4 = CumulusConstants.IPV4_ADDRESS_LINE.replaceAll( "\\{address\\}", mclag.getIpAddress() );
                }
            }

            /* First create the interface stanza */
            String peerlinkStanza =
                CumulusConstants.CLAG_STANZA.replaceAll( "\\{name\\}",
                                                         mclag.getInterfaceName() ).replaceAll( "\\{ipv4\\}",
                                                                                                ipv4 ).replaceAll( "\\{sharedMac\\}",
                                                                                                                   mclag.getSharedMac() ).replaceAll( "\\{peerIp\\}",
                                                                                                                                                      mclag.getPeerIp() ).replaceAll( "\\{enabled\\}",
                                                                                                                                                                                      mclag.isEnabled()
                                                                                                                                                                                                      ? "yes"
                                                                                                                                                                                                      : "no" );
            ...
            /* Start the interface */
            mclagCommand =
                CumulusConstants.ENABLE_MCLAG_COMMAND.replaceAll( "\\{password\\}",
                                                                  switchNode.getPassword() ).replaceAll( "\\{interfaces\\}",
                                                                                                         mclag.getInterfaceName() );
            ...
            if ( result.getExitCode() != 0 )
            {
                throw new HmsException( "Failed to enable MC-LAG on switch " + switchNode.getSwitchId()
                    + " with interface " + mclag.getInterfaceName() );
            }
        }
        else
        {
            mclagCommand =
                CumulusConstants.DISABLE_MCLAG_COMMAND.replaceAll( "\\{password\\}",
                                                                   qr( switchNode.getPassword() ) ).replaceAll( "\\{filename\\}",
                                                                                                                CumulusUtil.getMclagFilename() );
            ...
            if ( result.getExitCode() != 0 )
            {
                throw new HmsException( "Failed to disable MC-LAG on switch " + switchNode.getSwitchId() + " interface "
                    + mclag.getInterfaceName() );
            }
        }
        ...
        throws HmsOobNetworkException
    {
        ...
        /*
         * Only concentrate on the switch ports. Management port should not be configured via bulk configuration methods
         */
        ...
        throws HmsOobNetworkException
    {
        ...
        /*
         * TODO: Since we do not support configuring default route through SVI I am disabling that feature Once we have
         * that support it will be handled inside routeHelper.configureIpv4DefaultRoute(switchNode, gateway, portId);
         */
        try
        {
            int vid = Integer.parseInt( portId );

            if ( vid > 0 && vid <= 4096 )
            {
                logger.warn( String.format( "Not configuring default route on switch %s through SVI for VLAN ID %d since it is not supported",
                                            switchNode.getSwitchId(), vid ) );
                return; // this is a vlan id and we do not proceed further. We do not signal error since we do not want
                        // to alarm the caller for now, this feature is pending only for this plugin as of now
            }
        }
        catch ( Exception e )
        {
            // this means that it is not a VLAN id and we can safely proceed to the function
        }
    ...
    /**
     * Qr function for quote replacement Using a quote replacement tool on a string - returns literal string.
     * 
    ...
    /**
     * Find pattern in an array of strings Tries to find a regular expression pattern inside an array of strings The
     * complexity of search operation is O(N) and return the index in the array matching the pattern
     *
     ...
     */
    private static int ArrayFindPatternIndex( String[] haystack, String needle, String abortPattern, int startIndex,
                                              int endIndex )
        throws HmsException
    {
        ...
        if ( ( startIndex < 0 ) || ( endIndex > haystack.length ) || ( startIndex > endIndex ) )
            throw new HmsException( "Condition: 0 <= startIndex <= endIndex < haystack.length : Violated!" );
    ...
    /**
     * Functionality - efficient tool to call a set of commands and capture their results. This function is a time
     * saver. When with no dependency among each other a set of 'N' commands are to be fired and each of their output
     * stored for future analysis this function can do the work.
     *
     ...
     * @return An array of array of strings. The output of each command is given as array of strings. So it is an array
     *         of such arrays. NULL, if any abnormal situation like SSH termination, improper un-parsable output is
     *         encountered
     */
    ...
    /**
     * Upload temp configuration file Function to upload local file into /etc/network/interfaces via a temporary file
     * since this file has rw permission only for root, for other users it has only r permission
     *
     ...
     */
    private static void uploadConfigFile( SwitchSession switchSession, SwitchNode switchNode, String localFilename,
        ...
        throws HmsException
    {
        ...
        try
        {
            ...
            String r =
                switchSession.execute( "echo '{password}' | sudo -S mv -f {tmpfile} {interfaces}".replaceAll( "\\{password\\}",
                                                                                                              switchNode.getPassword() ).replaceAll( "\\{tmpfile\\}",
                                                                                                                                                     tmpCumulusFile ).replaceAll( "\\{interfaces\\}",
                                                                                                                                                                                  remoteFilename ) );
    ...
    /**
     * Function to configure OSPF parameters inside /etc/quagga/Quagga.conf only when OSPF is to be enabled with the
     * parameters present inside 'TorSwitchOspfConfig ospf' object passed. This function attempts to retain file section
     * positional sanctity. This function can be used to create brand new configuration if none exists but if existing
     * configuration is found it smartly merges the new configuration into the existing one without deleting existing
     * interface and network parameters. Global parameters however are overwritten with new values in 'ospf'.
     *
     ...
     */
    private void configureOspfParameters( SwitchNode switchNode, SwitchOspfConfig ospf )
        throws HmsException
    {
        int index = 0;
        SwitchSession switchSession = getSession( switchNode );
         ...
         */
        if ( ( ospf.getGlobal() == null )
            || ( ( ospf.getGlobal().getInterfaces() == null ) && ( ospf.getGlobal().getNetworks() == null ) ) )
        {
         ...
         */
        rtrIdLineIndex = headerLineCreate( quaggaConfig, 0, "^router-id.*", GLOBAL_ROUTER_ID_LINE,
                                           persistentNewLinesQuaggaHeader, true );
        rtrOspfLineIndex = headerLineCreate( quaggaConfig, 0, "^router ospf.*", GLOBAL_ROUTER_OSPF_LINE,
                                             persistentNewLinesQuaggaFooter, false );
        ospfRtrIdLineIndex = headerLineCreate( quaggaConfig, rtrIdLineIndex + 1, ".*ospf router-id.*",
                                               GLOBAL_OSPF_ROUTER_ID_LINE, persistentNewLinesQuaggaFooter, true );
        passiveInterfaceLineIndex = headerLineCreate( quaggaConfig, rtrIdLineIndex + 1, ".*passive-interface default.*",
                                                      GLOBAL_PASSIVE_MODE_LINE, null, true );
        ...
        if ( ( rtrIdLineIndex == -1 )
            && ( ( rtrOspfLineIndex != -1 ) || ( ospfRtrIdLineIndex != -1 ) || ( passiveInterfaceLineIndex != -1 ) ) )
        {
            throw new HmsException( "Please delete " + CumulusConstants.CUMULUS_QUAGGA_CONFIG_FILE
                + " and rerun the query" );
        }
        ...
        for ( SwitchOspfInterfaceConfig ifconfig : torSpineInterfaces )
        {
             ...
             */
            String[] patternsQ = { ".*ip ospf network.*" };
            String[] substValuesQ = { OSPF_INTERFACE_DEFAULT_TYPE };

            if ( ifconfig.getMode() == SwitchOspfInterfaceConfig.InterfaceMode.ACTIVE )
            {
                appendSectionInterfaces[index] =
            ...
            appendPatternsInterfaces[index] =
                ".*interface.*{ifName}.*".replaceAll( "\\{ifName\\}", ifconfig.getName() );

            int found = headerLineCreate( quaggaConfig, rtrIdLineIndex + 1,
                                          "^interface.*{ifName}.*".replaceAll( "\\{ifName\\}", ifconfig.getName() ),
                                          "!\ninterface {ifName}\n".replaceAll( "\\{ifName\\}", ifconfig.getName() ),
                                          persistentNewLinesQuaggaInterfaces, true );

            String av = headerSubsectionCreate( quaggaConfig, found + 1, patternsQ, substValuesQ, ".*interface.*" );
        ...
        for ( SwitchOspfNetworkConfig networkconfig : torSpineNetworks )
        {
            ...
            appendSectionNetworks[index] =
                " network {network} area {areaId}\n".replaceAll( "\\{network\\}",
                                                                 networkconfig.getNetwork() ).replaceAll( "\\{areaId\\}",
                                                                                                          networkconfig.getArea() );
            appendPatternsNetworks[index] =
                ".*network {networkName}.*".replaceAll( "\\{networkName\\}", networkconfig.getNetwork() );
            ++index;
        }
         ...
         */
        String appendValueOspfNetworks = headerSubsectionCreate( quaggaConfig, rtrSubSectionInsertLineIndex + 1,
                                                                 appendPatternsNetworks, appendSectionNetworks, null );
        String appendValueOspfInterfaces =
            headerSubsectionCreate( quaggaConfig, rtrSubSectionInsertLineIndex + 1, appendPatternsInterfaces,
                                    appendSectionInterfaces, null );
        ...
        else if ( ( appendValueOspfNetworks.length() > 0 ) || ( appendValueOspfInterfaces.length() > 0 ) )
        {
            ...
            quaggaConfig[rtrSubSectionInsertLineIndex] = quaggaConfig[rtrSubSectionInsertLineIndex].concat( "\n"
                + appendValueOspfInterfaces + appendValueOspfNetworks );
        }

        /*
         * Open a temporary file to store all persistent configuration This file will be uploaded at the end after
        ...
        uploadConfigFile( switchSession, switchNode, LOCAL_QUAGGA_CONFIG_FILE,
    ...
    /**
     * Creates loopback (acts as wrapper) Function to create IP Loopback Interface with a specific name and IP address
    ...
    private void createLoopback( String[] persistentConfigFile, List<String> newLines, String loopbackName,
                                 String loopbackIp )
        throws HmsException
    {
        createInterface( persistentConfigFile, newLines, loopbackName, "", loopbackIp, 32 );
    }

    /**
     * This function can be used to create associate an IPv4 address with a port We don't care if the port name is a
     * physical one or a logical one.
     *
     * @param persistentConfigFile An array of String (lines) representing the contents of /etc/network/interfaces
     * @param newLines If new lines are to be added to create the interface then they will be added to this list
     * @param ifName name of the port/interface
     * @param ifQualifier any additional option we need to put next to the interface name in line "iface swpx" e.g.
     *            iface swp5 inet manual, here this arg should be set to " inet manual"
     ...
     * @return void
     * @exception HmsException If invalid argument is passed or something unexpected happens during the function
     *                execution
     */
    private void createInterface( String[] persistentConfigFile, List<String> newLines, String ifName,
                                  String ifQualifier, String ifIp, int ifMaskLen )
        throws HmsException
    {
        int index = -1;
        ...
        index = ArrayFindPatternIndex( persistentConfigFile, ".*auto {ifName}.*".replaceAll( "\\{ifName\\}", ifName ),
                                       null, 0, -1 );
        ...
        if ( index == -1 )
        {
            newLines.add( ( "auto {ifName}\n" + "iface {ifName}{ifQualifier}\n"
                + "\taddress {ifIp}/32\n" ).replaceAll( "\\{ifName\\}",
                                                        ifName ).replaceAll( "\\{ifQualifier\\}",
                                                                             ifQualifier ).replaceAll( "\\{ifIp\\}",
                                                                                                       ifIp ) );
        }
        else
        {
            ...
            String[] substitutionStrings = {
                "iface {ifName}{ifQualifier}\n".replaceAll( "\\{ifName\\}", ifName ).replaceAll( "\\{ifQualifier\\}",
                                                                                                 ifQualifier ),
                "\taddress {ifIp}/{ifMaskLen}\n".replaceAll( "\\{loopbackIp\\}",
                                                             ifIp ).replaceAll( "\\{ifMaskLen\\}",
                                                                                String.valueOf( ifMaskLen ) ) };
            String stopString = ".*auto.*";

            appendLines =
                headerSubsectionCreate( persistentConfigFile, index + 1, patterns, substitutionStrings, stopString );

            if ( appendLines.length() > 0 )
            {
                persistentConfigFile[index] = persistentConfigFile[index].concat( "\n" + appendLines );
            }
        }
    }

    /**
     * Create a subsection following principle of set intersection as follows: Set A = set of lines in file Set B = set
     * of patterns passed Set C = set of substitution values C and B are mapped one-to-one set D = A ^ B --> set of
     * matching patterns (^ signifies intersection) set E = B - D --> set of patterns which did not match For elements
     * in C with matching indices in E we return them as a single concatenated String The search starts from the index
     * 'srchFromIndex'
     *
     ...
     */
    private String headerSubsectionCreate( String[] file, int srchFromIndex, String[] patterns,
                                           String[] substitutionStrings, String stopString )
        throws HmsException
    {
        ...
        if ( patterns.length != numSubstitutions )
            throw new HmsException( "Both patterns and substitutionStrings array MUST be of same length" );

        for ( int i = 0; i < numSubstitutions; ++i )
        {
            index = ArrayFindPatternIndex( file, patterns[i], stopString, srchFromIndex, -1 );
     ...
     * substitutes the passed 'substitutionString' into the matching index of the array. The substitution happens only
     * when 'skipIfFound' is set to false. If no match is found then it may also add the 'substitutionString' into the
     * linked list 'newlyAddedLines' but only when it is not NULL. 'pattern' can be any regular expression.
     *
     ...
     */
    private int headerLineCreate( String[] file, int srchFromIndex, String pattern, String substitutionString,
                                  List<String> newlyAddedLines, Boolean skipIfFound )
        throws HmsException
    {
        int hdrLine = -1;
        int index = ArrayFindPatternIndex( file, pattern, null, srchFromIndex, -1 );
    ...
    private void error( String msg, Exception e, HmsOobNetworkErrorCode error )
        throws HmsOobNetworkException
    {
        String finalMsg = msg;
    ...
    private void error( String msg, HmsOobNetworkErrorCode error )
        throws HmsOobNetworkException
    {
    ...
    private CumulusPortStatusHelper portStatusHelper = new CumulusPortStatusHelper( this );
    ...
    /**
     * Get Component Event List Get Sensor list (component event list) for a particular sensor component.
     ...
     * @exception HmsException Thrown if Component Sensor List fails to be retrieved for provided component
     */
    @Override
        ...
        throws HmsException
    {
        ...
        if ( serviceNode != null )
        {
            ...
            try
            {
                ...
                {
                    case SWITCH_PORT:
                        serverComponentEventList =
                            CumulusPortEventHelper.getSwitchPortEventHelper( new SwitchNode( serviceNode ),
                                                                             getSwitchPortList( new SwitchNode( serviceNode ) ) );
                        break;
                    case SWITCH:
                        serverComponentEventList =
                            CumulusSwitchUpDownEventHelper.getSwitchUpDownEventHelper( new SwitchNode( serviceNode ),
            ...
            {
                logger.error( "Exception while getting getComponentSensorList Data for switch:"
                    + serviceNode.getNodeID(), e );
                throw e;
            }
            catch ( Exception e )
            {
    ...
    /**
     * Get Supported Hms Apis Get the supported HMS Apis from constant variable. (Switch apis: info, port info, sensor
     * info, port sensor info, fan sensor info, powerunit info)
     *
     * @param serviceNode service node object
     * @return list of HMS APIs
     * @exception HmsException Thrown if static API values changed, and fails to meet API values
     */
    @Override
    ...
    public void setSwitchTime( SwitchNode switchNode, long time )
        throws HmsOobNetworkException
    {
        // we calculate the time with seconds as the lowest granularity since that is sufficient
        // for this case. There is already enough lag when the time value arrives at this API from VRM
        CumulusTorSwitchSession session = (CumulusTorSwitchSession) getSession( switchNode );
        ...
        if ( time == 0 )
            error( "Cannot set zero time", null, HmsOobNetworkErrorCode.ARGUMENT_SYNTAX_ERROR );

        /* Change the current settings on the switch and this command also sets the time in HW clock */
        String setTimeCommand =
            CumulusConstants.SET_TIME_COMMAND.replaceAll( "\\{password\\}",
                                                          qr( switchNode.getPassword() ) ).replaceAll( "\\{seconds\\}",
                                                                                                       Long.toString( time ) );

        try
        {
            SshExecResult result = session.executeEnhanced( setTimeCommand );

            if ( result != null && result.getExitCode() != 0 && result.getStderrAsString() != null )
            {
                error( "Failed to set time. Reason : " + result.getStderrAsString(),
                       HmsOobNetworkErrorCode.SET_OPERATION_FAILED );
            }
        }
        catch ( HmsException e )
        {
            error( "Failed to set time. Reason : " + e.getMessage(), HmsOobNetworkErrorCode.SET_OPERATION_FAILED );
        }
    }

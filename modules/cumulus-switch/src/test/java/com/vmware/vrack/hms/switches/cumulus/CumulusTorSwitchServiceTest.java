...
public class CumulusTorSwitchServiceTest {
	...
	public void init() {
	    ...
	    try {
	    	...
	    	switchNode_Obj = new SwitchNode("S0", "SSH", properties.getProperty("deviceIp"), new Integer(22),
	...
	public void testInvalidLogin() {
		...
		try {
			...
			if(switchOsInfo != null) {
				System.out.println("[Device IP : "+deviceIp+"] Found Switch OS information = " + switchOsInfo.getOsName());
			} else {
	...
	public void testGetSwitchType(){
		...
		try {
			...
			if(switchType.compareTo(switchTypeRetrieved)==0){
				logger.info("SwitchType matches the function return value: " + cumulusTorSwitchService_Obj.getSwitchType());
			} else {
	...
	/**
	 * Get Switch Port: Get each switch port by using the switchport names list.
	 *
	 * More info: Confirm all attributes of the switchport are not null.
	 */
	@Test
	public void testGetSwitchPort(){
		...
		try {
			...
			for(int i=0; i<listOfSwitchPorts.size(); i++){
				logger.info("from list, get each i: " + i + " list of switchports value: " + listOfSwitchPorts.get(i));
				switchPort_Obj = cumulusTorSwitchService_Obj.getSwitchPort(switchNode_Obj, listOfSwitchPorts.get(i));
	...
	public void testGetSwitchVlan(){
		...
		try {
			...
			if(listOfVlansString != null && !listOfVlansString.isEmpty()){
				logger.info("Size of list of vlans: " + listOfVlansString.size());
				for(int i=0; i < listOfVlansString.size(); i++){
					switchVlan_Obj = cumulusTorSwitchService_Obj.getSwitchVlan(switchNode_Obj, listOfVlansString.get(i));
			...
			if(listOfSwitchVlans != null && !listOfSwitchVlans.isEmpty()){
				...
				if(printAll){
					for(int i=0; i < listOfSwitchVlans.size(); i++){
						...
						logger.info((listOfSwitchVlans.get(i).getTaggedPorts().isEmpty())
								? "taggedPorts is Empty" : "taggedPorts is NOT Empty");
		...
		try{
			if(listOfSwitchVlans == null) {
				...
				for(int vlan = 0; vlan < listOfSwitchVlans.size(); vlan++){
					logger.info("in here");
					createVlanSuccess = cumulusTorSwitchService_Obj.createVlan(switchNode_Obj, listOfSwitchVlans.get(vlan));
		...
		try {
			if(listOfLacpGroupNames == null) {
				...
				if(listOfLacpGroupNames != null && !listOfLacpGroupNames.isEmpty()) {
					...
					for(int i=0; i < listOfLacpGroupNames.size(); i++){
						switchLacpGroup_Obj = cumulusTorSwitchService_Obj.getSwitchLacpGroup(switchNode_Obj, listOfLacpGroupNames.get(i));

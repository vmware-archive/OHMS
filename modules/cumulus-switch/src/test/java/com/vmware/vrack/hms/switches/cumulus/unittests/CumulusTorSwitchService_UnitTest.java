    ...
    public void testGetSwitchType(){
        ...
        try {
            ...
            if(switchType.compareTo(switchTypeRetrieved)==0){
                logger.info("SwitchType matches the function return value: " + fakeISwitchService_Obj.getSwitchType());
            } else {
        ...
        try{
            if(listOfSwitchVlans == null) {
                ...
                for(int vlan = 0; vlan < listOfSwitchVlans.size(); vlan++){
                    logger.info("in here");
                    createVlanSuccess = fakeISwitchService_Obj.createVlan(switchNode_Obj, listOfSwitchVlans.get(vlan));
        ...
        try {
            if(listOfLacpGroupNames == null) {
                ...
                if(listOfLacpGroupNames != null && !listOfLacpGroupNames.isEmpty()) {
                    ...
                    for(int i=0; i < listOfLacpGroupNames.size(); i++){
                        switchLacpGroup_Obj = fakeISwitchService_Obj.getSwitchLacpGroup(switchNode_Obj, listOfLacpGroupNames.get(i));

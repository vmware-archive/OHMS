...
public class SshExecResult {
    ...
    public void logIfError(Logger logger) {
        if (exitCode != 0) {
            ...
            if (stderr.length > 0) logger.error ("stderr: " + new String (stderr));
            if (stdout.length > 0) logger.error ("stdout: " + new String (stdout));
        }
    }
    ...
    public void log(Logger logger, Level level) {
        ...
        if (stdout.length > 0) logger.log (level, "stdout: " + new String (stdout));
        if (stderr.length > 0) logger.log (level, "stderr: " + new String (stderr));
    }

...
public class SshUtil
{
    ...
    public static String maskedPasswordString( String command )
    {
        ...
        if ( matcher.matches() )
        {
            maskedString = command.replaceAll( Pattern.quote( matcher.group( 1 ) ), "********" );
        }
    ...
    /**
     * Add supplied host key in known host file.
     *
     * <pre>
     * NOTE:
     * The Session object that is passed to this method MUST be created with StrictHostKeyChecking set to No.
     * Otherwise, Session.connect will fail and HostKey will not be added to knownHostsFile.
     * </pre>
     *
     * @param knownHostFile
     * @param hostKey

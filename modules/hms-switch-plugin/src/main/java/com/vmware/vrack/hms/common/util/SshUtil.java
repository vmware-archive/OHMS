...
public class SshUtil {
	...
	public static String maskedPasswordString(String command) {
		...
		Pattern passwdPattern = Pattern
				.compile(".*echo\\s+('\\S+')\\s*\\|\\s*sudo\\s+-S\\s+.+$");
	...
	public  static Session getSessionObject(String userName, String password,
			String hostName, int port, Properties sessionConfig) {

		Session session = null;
		
		//	logger.debug("In getSessionObject");
		if(userName != null && !"".equals(userName) && 
		   password != null && hostName!=null && !"".equals(hostName))
		{
			JSch jsch = new JSch();
			try {
	...
	/**
	 * It executes the command and returns the response back to the calling function.
	 * 
	 * This function will expect Session object and command string as parameters .
	 *  
	 * @param sessionObj
	 * @param command
	 * @return
	 ...
	 * @throws SftpException
	 */
	public static SftpATTRS lstat (Session session, String remoteFile) throws JSchException, SftpException {
		ChannelSftp sftpChannel = (ChannelSftp) session.openChannel("sftp");
		sftpChannel.connect();
	...
	/**
	 * It executes the set of commands and returns the last response back to the calling function.
	 * 
	 * This function will expect Session object and an array of String[][] objects as
	 * parameters. It is an array of Strings where the format is as follows:
	 * 
	 * command[0] = <login prompt>
	 * command[1] = <send command-1>
	 * command[2] = <expected output>
	 * command[3] = <send command-2>
	 * command[4] = <expected output>
	 * 
	 * Thus there should always be odd number of elements in the array starting with index-0 holding the lgin prompt
	 * always.
	 * 
	 * The expectation should be exact match of how the expected prompt ends.\
	 * Send command need not have newline at its end.
	 *  
	 * @param sessionObj
	 * @param commands
	 * @return
		...
		if(sessionObj!=null && commands != null)
		{
			...
			try {
				...
				byte[] tmp = new byte[arrMaxSize];
					...
					if (channel.isClosed() && channel.getExitStatus() != 0) {
						logger.debug("Command exited with error code "

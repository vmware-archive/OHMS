...
{
    /**
     * Retrieve the netmask from prefix length. Example: Input: 24, Output: 255.255.255.0
     * 
     * @param prefixLen
     * @return
     */
    public static String prefixLenToNetmask( Integer prefixLen )
    {
        ...
        for ( Integer i = 3; i >= 0; --i )
        {
            ...
            /*
             * msb is set to 0 (to avoid invalid sign bit interpretation) The msb can never be 0 in a netmask hence it
             * is always safe to make that bit 1 when determining the mask for the most significant octet
             */
            if ( i == 3 )
                tmp |= 0x80;
            netmask += tmp;
    ...
    /**
     * Function to convert a valid netmask to its prefix length. Example: Input: 255.255.255.0, Output: 24
     *
     * @param netmask
     * @return
     */
    ...
    /**
     * Function to validate an IPv4 address presented to this function in dotted decimal notation as per

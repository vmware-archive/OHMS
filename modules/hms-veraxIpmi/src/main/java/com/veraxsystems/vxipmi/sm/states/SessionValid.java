 ...
 * <li>Sends {@link CloseSession} and transits to {@link Authcap} on
    ...
    extends State
{
    ...
    public void doTransition( StateMachine stateMachine, StateMachineEvent machineEvent )
    {
        ...
        else if ( machineEvent instanceof SessionUpkeep )
        {
            ...
            try
            {
                stateMachine.sendMessage( Encoder.encode( new Protocolv20Encoder(),
                                                          new GetChannelAuthenticationCapabilities( IpmiVersion.V20,
                                                                                                    IpmiVersion.V20,
                                                                                                    cipherSuite,
        ...
        {
            com.veraxsystems.vxipmi.sm.events.CloseSession event =
                (com.veraxsystems.vxipmi.sm.events.CloseSession) machineEvent;
            try
            {
                stateMachine.setCurrent( new Authcap() );
                stateMachine.sendMessage( Encoder.encode( new Protocolv20Encoder(),

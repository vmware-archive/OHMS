...
/**
 * A wrapper for IPMI command. Parameterless constructors in classes derived from IpmiCommandCoder are meant to be used
 * for decoding. To avoid omitting setting an important parameter when encoding message use parametered constructors
 * rather than the parameterless ones.
 */
public abstract class IpmiCommandCoder
{
     ...
     *
     * @param sequenceNumber - A generated sequence number used for matching request and response. If IPMI message is
     *            sent in a session, it is used as a Session Sequence Number. For all IPMI messages, sequenceNumber %
     *            256 is used as a IPMI LAN Message sequence number and as an IPMI payload message tag.
     * @param sessionId - ID of the managed system's session message is being sent in. For sessionless commands should b
     *            set to 0.
     * @return IPMI message
        ...
        throws NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        /* IPMI version 2.0 */ {
            ...
            message.setPayloadEncrypted( getCipherSuite().getConfidentialityAlgorithm().getCode() != SecurityConstants.CA_NONE );
            message.setPayload( preparePayload( sequenceNumber ) );

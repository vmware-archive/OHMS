...
{
    Nits( SensorUnit.NITS ),
    Lumen( SensorUnit.LUMEN ),
    Lux( SensorUnit.LUX ),
    Candela( SensorUnit.CANDELA ),
    kPa( SensorUnit.KPA ),
    Psi( SensorUnit.PSI ),
    Newton( SensorUnit.NEWTON ),
    CuIn( SensorUnit.CUIN ),
    Cfm( SensorUnit.CFM ),
    Cufeet( SensorUnit.CUFEET ),
    Rpm( SensorUnit.RPM ),
    mm( SensorUnit.MM ),
    Hz( SensorUnit.HZ ),
    cm( SensorUnit.CM ),
    m( SensorUnit.M ),
    CuCm( SensorUnit.CUCM ),
    CuM( SensorUnit.CUM ),
    Henry( SensorUnit.HENRY ),
    Liters( SensorUnit.LITERS ),
    Millihenry( SensorUnit.MILLIHENRY ),
    FluidOunce( SensorUnit.FLUIDOUNCE ),
    Farad( SensorUnit.FARAD ),
    Radians( SensorUnit.RADIANS ),
    Microfarad( SensorUnit.MICROFARAD ),
    Ohms( SensorUnit.OHMS ),
    Siemens( SensorUnit.SIEMENS ),
    Unspecified( SensorUnit.UNSPECIFIED ),
    Mole( SensorUnit.MOLE ),
    DegreesC( SensorUnit.DEGREESC ),
    Becquerel( SensorUnit.BECQUEREL ),
    Byte( SensorUnit.BYTE ),
    DegreesF( SensorUnit.DEGREESF ),
    PartsPerMilion( SensorUnit.PARTSPERMILION ),
    DegreesK( SensorUnit.DEGREESK ),
    Kilobyte( SensorUnit.KILOBYTE ),
    Megabyte( SensorUnit.MEGABYTE ),
    Volts( SensorUnit.VOLTS ),
    Gigabyte( SensorUnit.GIGABYTE ),
    Amps( SensorUnit.AMPS ),
    Word( SensorUnit.WORD ),
    Watts( SensorUnit.WATTS ),
    Dword( SensorUnit.DWORD ),
    Joules( SensorUnit.JOULES ),
    Qword( SensorUnit.QWORD ),
    Coulombs( SensorUnit.COULOMBS ),
    UncorrectableError( SensorUnit.UNCORRECTABLEERROR ),
    Line( SensorUnit.LINE ),
    Va( SensorUnit.VA ),
    FatalError( SensorUnit.FATALERROR ),
    Hit( SensorUnit.HIT ),
    Grams( SensorUnit.GRAMS ),
    Miss( SensorUnit.MISS ),
    Microsecond( SensorUnit.MICROSECOND ),
    Millisecond( SensorUnit.MILLISECOND ),
    Second( SensorUnit.SECOND ),
    Minute( SensorUnit.MINUTE ),
    Hour( SensorUnit.HOUR ),
    Day( SensorUnit.DAY ),
    Week( SensorUnit.WEEK ),
    Mil( SensorUnit.MIL ),
    Steradians( SensorUnit.STERADIANS ),
    Inches( SensorUnit.INCHES ),
    Revolutions( SensorUnit.REVOLUTIONS ),
    Feet( SensorUnit.FEET ),
    Cycles( SensorUnit.CYCLES ),
    Gravities( SensorUnit.GRAVITIES ),
    Ounce( SensorUnit.OUNCE ),
    Pound( SensorUnit.POUND ),
    FtLb( SensorUnit.FTLB ),
    Decibels( SensorUnit.DECIBELS ),
    OzIn( SensorUnit.OZIN ),
    DbA( SensorUnit.DBA ),
    Gauss( SensorUnit.GAUSS ),
    DbC( SensorUnit.DBC ),
    Gilberts( SensorUnit.GILBERTS ),
    Gray( SensorUnit.GRAY ),
    Sievert( SensorUnit.SIEVERT ),
    ColorTempDegK( SensorUnit.COLORTEMPDEGK ),
    bit( SensorUnit.BIT ),
    kilobit( SensorUnit.KILOBIT ),
    Retry( SensorUnit.RETRY ),
    Reset( SensorUnit.RESET ),
    Megabit( SensorUnit.MEGABIT ),
    OverrunOverflow( SensorUnit.OVERRUNOVERFLOW ),
    Gigabit( SensorUnit.GIGABIT ),
    Underrun( SensorUnit.UNDERRUN ),
    Collision( SensorUnit.COLLISION ),
    Packets( SensorUnit.PACKETS ),
    Messages( SensorUnit.MESSAGES ),
    Characters( SensorUnit.CHARACTERS ),
    Error( SensorUnit.ERROR ),
    CorrectableError( SensorUnit.CORRECTABLEERROR ),
    Other( SensorUnit.OTHER ),;

...
public enum ReadingType
{
     ...
     */
    FruInactive( ReadingType.FRUINACTIVE ),
    SlotConnectorIdentifyStatusAsserted( ReadingType.SLOTCONNECTORIDENTIFYSTATUSASSERTED ),
    HardReset( ReadingType.HARDRESET ),
    /**
     * Switch indicating FRU latch is in 'unlatched' position and FRU is mechanically removable
     */
    FruLatchOpen( ReadingType.FRULATCHOPEN ),
    FruActivationRequested( ReadingType.FRUACTIVATIONREQUESTED ),
    SlotConnectorDeviceInstalled( ReadingType.SLOTCONNECTORDEVICEINSTALLED ),
    WarmReset( ReadingType.WARMRESET ),
    FruActivationInProgress( ReadingType.FRUACTIVATIONINPROGRESS ),
    /**
    ...
    /**
     * Returns the byte code of the discrete sensor reading
     * 
     * @return List<Integer> For example: If the return value is 880384 (0x0d6f00) 0D - Sensor Type Code (Drive
     *         Slot/Bay) 6f - Sensor Specific Reading type 00 - Sensor specific Offset (Drive Presence)
     **/
    public static int byteCode( SensorType sensorType, int eventReadingType, int offset )
    {
     ...
     * 
     * @param sensorType - {@link SensorType} of the sensor
     * @param eventReadingType - value received via {@link FullSensorRecord#getEventReadingType()},
     *            {@link CompactSensorRecord#getEventReadingType()} or {@link EventOnlyRecord#getEventReadingType()}
     * @param offset - index of the state asserted
    ...
    public static ReadingType parseInt( SensorType sensorType, int eventReadingType, int offset )
    {
        ...
        switch ( value )
        {
            ...
            default:
                logger.warn( "Invalid value: " + value + " (" + Integer.toHexString( value ) + ") for sensor "
                    + sensorType );
                return Unknown;
        }
    }

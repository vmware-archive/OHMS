    ...
    implements MachineObserver
{
     ...
     *
     * @param messenger - {@link Messenger} associated with the proper {@link Constants#IPMI_PORT}
     ...
     */
    public List<CipherSuite> getAvailableCipherSuites( int tag )
        throws Exception
    {
        ...
        throws Exception
    {
        ...
        if ( !( lastAction instanceof ResponseAction || lastAction instanceof GetSikAction ) )
        {
            ...
            throw new ConnectionException( "Invalid StateMachine response: " + lastAction.getClass().getSimpleName() );
        }
    }
     ...
     * @throws Exception when sending message to the managed system fails
     */
    public GetChannelAuthenticationCapabilitiesResponseData getChannelAuthenticationCapabilities( int tag,
                                                                                                  CipherSuite cipherSuite,
                                                                                                  PrivilegeLevel requestedPrivilegeLevel )
                                                                                                      throws Exception
    {
        if ( isEncryptMessage() && !( stateMachine.getCurrent().getClass() == Ciphers.class ) )
        {
            throw new ConnectionException( "Illegal connection state: "
                + stateMachine.getCurrent().getClass().getSimpleName() );
        }
     ...
     * {@link #getChannelAuthenticationCapabilities(int, CipherSuite, PrivilegeLevel)} or {@link #closeSession()}
     *
     * @param tag - the integer from range 0-63 to match request with response
                                  ...
                                  throws Exception
    {
        ...
        lastAction = null;
        // Open Session
        stateMachine.doTransition( new Authorize( cipherSuite, tag, privilegeLevel, sessionId, this ) );
        waitForResponse();
        ResponseAction action = (ResponseAction) lastAction;
        lastAction = null;
        ...
        stateMachine.doTransition( new DefaultAck() );
        // RAKP 1
        stateMachine.doTransition( new OpenSessionAck( cipherSuite, privilegeLevel, tag, managedSystemSessionId,
                                                       username, password, bmcKey ) );
        waitForResponse();
        ...
        stateMachine.doTransition( new DefaultAck() );
        // RAKP 3
        stateMachine.doTransition( new Rakp2Ack( cipherSuite, tag, (byte) 0, managedSystemSessionId,
                                                 rakp1ResponseData ) );
        waitForResponse();
        action = (ResponseAction) lastAction;
        if ( sik == null )
     ...
     * @return ID of the message that will be also attached to the response to pair request with response if queue was
     *         not full and message was sent, -1 if sending of the message failed.
     * @throws ConnectionException when connection isn't in state where sending commands is allowed
     * @throws ArithmeticException when {@link Connection} runs out of available ID's for the messages. If this happens
    ...
    /**
     * Attempts to retry sending a message (message will be sent only if current number of retries does not exceed and
     * is not equal to maxAllowedRetries. <br>
     * IMPORTANT <br>
        ...
        throws NullPointerException
    {
        ...
        {
            lastMessageRecieveTimeStamp = System.currentTimeMillis();
            IpmiCommandCoder coder =
                messageQueue.getMessageFromQueue( ( (IpmiLanResponse) message.getPayload() ).getSequenceNumber() );
    ...
    /**
     * {@link TimerTask} runner - periodically sends no-op messages to keep the session up
     */
    @Override
    public void run()
    {
        ...
        do
        {
            try
            {
                ...
                // no need to send a heart-beat/ping if any other message has been sent.
                // Example. If pingPeriod is set to 30secs,
                // before sending heart-beat it will make sure no message has been received in last 30 seconds
                if ( System.currentTimeMillis() - lastMessageRecieveTimeStamp >= pingPeriod )
                    result =
                        sendIpmiCommand( new com.veraxsystems.vxipmi.coding.commands.session.GetChannelAuthenticationCapabilities( IpmiVersion.V20,
                                                                                                                                   IpmiVersion.V20,
            ...
            {
                logger.error( "---- session keep alive exception ---- " + e.getMessage(), e );
            }
        }
        while ( result <= 0 );
    }

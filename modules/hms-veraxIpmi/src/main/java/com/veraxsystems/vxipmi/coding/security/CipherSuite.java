...
public class CipherSuite
{
    ...
    public void initializeAlgorithms( byte[] sik )
        throws InvalidKeyException, IllegalArgumentException, NoSuchAlgorithmException, NoSuchPaddingException
    {
    ...
    /**
     * Builds Cipher Suites collection from raw data received by {@link GetChannelCipherSuites} commands. Cannot be
     * executed in {@link GetChannelCipherSuitesResponseData} since data comes in 16-byte packets and is fragmented.
    ...
    public static List<CipherSuite> getCipherSuites( byte[] bytes )
    {
        ...
        while ( offset < bytes.length )
        {
            ...
            byte ia = -1;
            ++offset;
            while ( offset < bytes.length && bytes[offset] != TypeConverter.intToByte( 0xC0 )

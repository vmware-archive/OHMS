    ...
    extends IpmiCommandCoder
{
    ...
    /**
     * Initiates class for decoding. Sets IPMI version to {@link IpmiVersion#V20} since OpenSession is a RMCP+ command.
     * Sets authentication type to RMCP+.
     *
     * @param cipherSuite - {@link CipherSuite} containing authentication, confidentiality and integrity algorithms for
     *            this session. Cipher Suite might (and probably will be, because at this point of session creation SIK
     *            is not yet known) be not initialized.
     * @see IpmiVersion
     */
    ...
    /**
     * Initiates class for encoding and decoding. Sets IPMI version to {@link IpmiVersion#V20} since OpenSession is a
     * RMCP+ command. Sets authentication type to RMCP+.
     *
     * @see IpmiVersion
     * @param sessionID - Session ID selected by the remote console.
     * @param privilegeLevel - Requested privilege level for the session. Can be {@link PrivilegeLevel#MaximumAvailable}
     *            .
     * @param cipherSuite - {@link CipherSuite} containing authentication, confidentiality and integrity algorithms for
     *            this session. Cipher Suite might (and probably will be, because at this point of session creation SIK
     *            is not yet known) be not initialized.
    ...
    public OpenSession( int sessionID, PrivilegeLevel privilegeLevel, CipherSuite cipherSuite, Connection connection )
    {
        this( sessionID, privilegeLevel, cipherSuite );

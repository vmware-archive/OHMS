    ...
    extends IpmiCommandCoder
{
    ...
    /**
     * Initiates ReadFruData for both encoding and decoding. Sets session parameters to default.
     *
     * @see IpmiCommandCoder#setSessionParameters(IpmiVersion, CipherSuite, AuthenticationType)
     * @param fruId - ID of the FRU to get info from. Must be less than 256. To get FRU ID use {@link GetSdr} to
     *            retrieve {@link FruDeviceLocatorRecord}.
     * @param unit - {@link BaseUnit} indicating if the FRU device is accessed in {@link BaseUnit#Bytes} or
     *            {@link BaseUnit#Words}
     * @param offset - offset to read in units specified by unit
     * @param countToRead - size of the area to read in unit. Cannot exceed 255;
     */
     ...
     * @param countToRead - size of the area to read in unit. Cannot exceed 255;
     */
    public ReadFruData( IpmiVersion version, CipherSuite cipherSuite, AuthenticationType authenticationType, int fruId,
                        BaseUnit unit, int offset, int countToRead )
    {
    ...
    /**
     * Decodes {@link FruRecord}s from data provided by {@link ReadFruData} command. Size of the FRU Inventory Area
     * might exceed size of the communication packet so it might come in many {@link ReadFruDataResponseData} packets.
     *
     * @param fruData - list of {@link ReadFruDataResponseData} containing FRU data
    ...
    public static List<FruRecord> decodeFruData( List<ReadFruDataResponseData> fruData )
    {
        ...
        if ( data[0] == 0x1 )
        {
            ...
            if ( multiRecordOffset != 0 )
            {
                ...
                {
                    list.add( MultiRecordInfo.populateMultiRecord( data, multiRecordOffset ) );
                    multiRecordOffset += TypeConverter.byteToInt( data[multiRecordOffset + 2] ) + 5;
                }
            }
        }
        else if ( false )

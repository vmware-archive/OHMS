...
/**
 * <p>
 * A wrapper for RMCP+ RAKP1 message and it's response - RAKP2 message. The same instance of this class that was used to
 * prepare RAKP Message 1 should be also used to decode matching RAKP Message 2 since the generated random number is
 * used in encryption process.
 * </p>
 * <p>
 * Capable of calculating SIK (Session Integrity Key) when RAKP Message 2 is given (
 * {@link #calculateSik(Rakp1ResponseData)}).
 * </p>
 */
public class Rakp1
    extends IpmiCommandCoder
{
    ...
    /**
     * ASCII character Name that the user at the Remote Console wishes to assume for this session. It's length cannot
     * exceed 16.
     */
    ...
    /**
     * Kg key associated with the target BMC. Should be null if Get Channel Authentication Capabilities Response
     * indicated that Kg is disabled which means that 'one-key' logins are being used (
        ...
        throws IllegalArgumentException, IPMIException, NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        if ( !getCipherSuite().getAuthenticationAlgorithm().checkKeyExchangeAuthenticationCode( prepareKeyExchangeAuthenticationCodeBase( data ),
                                                                                                key, getPassword() ) )
        {
        ...
        throws InvalidKeyException, NoSuchAlgorithmException
    {
        ...
        return getCipherSuite().getAuthenticationAlgorithm().getKeyExchangeAuthenticationCode( prepareSikBase( responseData ),
                                                                                               new String( key ) );
    }

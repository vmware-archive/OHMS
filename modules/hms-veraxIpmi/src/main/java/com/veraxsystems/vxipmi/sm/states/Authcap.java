...
/**
 * {@link GetChannelAuthenticationCapabilities} response was received. At this point the Session Challenge is going to
 * start. Transits to {@link OpenSessionWaiting} on {@link Authorize}.
 */
    ...
    public void doTransition( StateMachine stateMachine, StateMachineEvent machineEvent )
    {
        if ( machineEvent instanceof Authorize )
        {
            ...
            OpenSession openSession = new OpenSession( event.getSessionId(), event.getPrivilegeLevel(),
                                                       event.getCipherSuite(), event.getConnection() );
            try
            {

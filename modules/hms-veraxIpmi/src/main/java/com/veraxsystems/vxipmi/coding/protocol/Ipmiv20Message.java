    ...
    extends IpmiMessage
{
    ...
    /**
     * Gets base for integrity algorithm calculations. If used for the message being created, does not contain AuthCode
     * field so amount of Integrity Pad bytes cannot be calculated. Therefore integrity algorithms using this base must
     * add proper amount of the Integrity Pad bytes and modify the Pad Length byte.
     *
     * @param encoder - {@link IpmiEncoder} to be used to convert message to byte array format. Must be able to handle
     *            null authCode field with {@link #isPayloadAuthenticated()} == true since this method is used in its
     *            generation.
     ...
     */
    public byte[] getIntegrityAlgorithmBase( IpmiEncoder encoder )

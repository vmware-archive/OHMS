    ...
    public BoardInfo( byte[] fruData, int offset )
    {
        ...
        try
        {
            setMfgDate( new Date( df.parse( "01/01/96" ).getTime()
                + ( (long) TypeConverter.littleEndianByteArrayToInt( buffer ) ) * 60000l ) );
        }
        ...
        while ( partNumber != 0xc1 && offset < fruData.length )
        {
            ...
            if ( partDataLength > 0 && partDataLength + offset < fruData.length )
            {
                ...
                {
                    case 0:
                        setBoardManufacturer( FruRecord.decodeString( partType, partNumberData,
                                                                      languageCode != 0 && languageCode != 25 ) );
                        break;
                    case 1:
                        setBoardProductName( FruRecord.decodeString( partType, partNumberData,
                                                                     languageCode != 0 && languageCode != 25 ) );
                        break;
                    case 2:
                        setBoardSerialNumber( FruRecord.decodeString( partType, partNumberData, true ) );
                        break;
                    case 3:
                        setBoardPartNumber( FruRecord.decodeString( partType, partNumberData,
                                                                    languageCode != 0 && languageCode != 25 ) );
                        break;
                    case 4:

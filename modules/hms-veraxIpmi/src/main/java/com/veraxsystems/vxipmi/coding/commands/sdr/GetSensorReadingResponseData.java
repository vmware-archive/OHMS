    ...
    implements ResponseData
{
    ...
    /**
     * This bit is set to indicate that a 're-arm' or 'Set Event Receiver' command has been used to request an update of
     * the sensor status, and that update has not occurred yet. Software should use this bit to avoid getting an
     * incorrect status while the first sensor update is in progress.
     */
     ...
     * Contains state of the sensor if it is discrete.
     *
     * @param sensorEventReadingType - value received via {@link FullSensorRecord#getEventReadingType()},
    ...
    /**
     * Contains byte codes of the sensor if it is discrete.
     * 
     ...
     *            {@link CompactSensorRecord#getEventReadingType()} or {@link EventOnlyRecord#getEventReadingType()}
     */
    public List<Integer> getByteCodeAsserted( SensorType sensorType, int sensorEventReadingType )
    {
        ...
        for ( int i = 0; i < statesAsserted.length; ++i )
        {
            if ( statesAsserted[i] )
            {
                list.add( ReadingType.byteCode( sensorType, sensorEventReadingType, i ) );
            }
        }
        return list;
    }

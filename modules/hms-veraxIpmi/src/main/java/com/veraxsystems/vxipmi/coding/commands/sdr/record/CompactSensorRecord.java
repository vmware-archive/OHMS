 ...
 */
public class CompactSensorRecord
    extends SensorRecord
{
    ...
    /**
     * The instance modifier is a character(s) that software can append to the end of the ID String. This field selects
     * whether the appended character(s) will be numeric or alpha.
     */
    private InstanceModifierType idInstanceModifierType;

    /**
     * Sensor numbers sharing this record are sequential starting with the sensor number specified by the Sensor Number
    ...
    protected void populateTypeSpecficValues( byte[] recordData, SensorRecord record )
    {
        ...
        setSensorDirection( SensorDirection.parseInt( ( TypeConverter.byteToInt( recordData[23] ) & 0xc0 ) >> 6 ) );
        setIdInstanceModifierType( InstanceModifierType.parseInt( ( TypeConverter.byteToInt( recordData[23] )
            & 0x30 ) >> 4 ) );

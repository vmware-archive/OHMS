 ...
 * {@link Timeout}, further proceeds with getting Cipher Suites on {@link GetChannelCipherSuitesPending} and moves on to
    ...
    extends State
{
    ...
    public void doTransition( StateMachine stateMachine, StateMachineEvent machineEvent )
    {
        ...
        else if ( machineEvent instanceof GetChannelCipherSuitesPending )
        {
            ...
            GetChannelCipherSuites cipherSuites =
                new GetChannelCipherSuites( TypeConverter.intToByte( 0xE ), TypeConverter.intToByte( index + 1 ) );
            try
            {
    ...
    public void doAction( StateMachine stateMachine, RmcpMessage message )
    {
        ...
        try
        {
            ...
            // System.out.println("[CW " + stateMachine.hashCode() + "] Expected: " + tag + " encountered: " +
            // TypeConverter.byteToInt(((IpmiLanResponse) ipmiMessage
            // .getPayload()).getSequenceNumber()));
            if ( suites.isCommandResponse( ipmiMessage )
                && TypeConverter.byteToInt( ( (IpmiLanResponse) ipmiMessage.getPayload() ).getSequenceNumber() ) == tag )
            {

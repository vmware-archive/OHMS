...
/**
 * At this point of session challenge, RAKP Message 3 was sent, {@link StateMachine} is waiting for RAKP Message 4.<br>
 * Transition to:
 * <li>{@link Rakp3Complete} on {@link DefaultAck}
    ...
    extends State
{
     ...
     * Initiates state.
     *
     * @param rakp1 - the {@link Rakp1} message that was sent earlier in the authentification process.
     * @param rakp1ResponseData - the {@link Rakp1ResponseData} that was received earlier in the authentification
     *            process.
    ...
    public void doAction( StateMachine stateMachine, RmcpMessage message )
    {
        ...
        try
        {
            ...
            /*
             * System.out.println("[R3W " + stateMachine.hashCode() + "] Expected: " + tag + " encountered: " +
             * TypeConverter.byteToInt(((PlainMessage) ipmiMessage .getPayload()).getPayloadData()[0]));
             */
            if ( rakp3.isCommandResponse( ipmiMessage )
                && TypeConverter.byteToInt( ( (PlainMessage) ipmiMessage.getPayload() ).getPayloadData()[0] ) == tag )
            {

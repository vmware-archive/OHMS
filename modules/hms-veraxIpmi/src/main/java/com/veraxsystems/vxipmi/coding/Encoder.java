...
public final class Encoder
{
     ...
     *
     * @param protcolEncoder - instance of {@link IpmiEncoder} class for encoding of the IPMI session header.
     *            {@link Protocolv15Decoder} or {@link Protocolv20Decoder} should be used (depending on IPMI protocol
     *            version used).
     ...
     * @param sessionId - ID of the managed system's session message is being sent in. For sessionless commands should
     *            be set to 0.
     ...
     */
    public static byte[] encode( IpmiEncoder protcolEncoder, IpmiCommandCoder commandCoder, int sequenceNumber,
                                 int sessionId )
                                     throws NoSuchAlgorithmException, InvalidKeyException
    {
        return RmcpEncoder.encode( new RmcpIpmiMessage( protcolEncoder.encode( commandCoder.encodeCommand( sequenceNumber,
                                                                                                           sessionId ) ) ) );
    }

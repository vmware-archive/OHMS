    ...
    extends SensorRecord
{
    ...
    protected void populateTypeSpecficValues( byte[] recordData, SensorRecord record )
    {
        ...
        setSensorType( SensorType.parseInt( TypeConverter.byteToInt( recordData[10] ) ) );
        setEventReadingType( TypeConverter.byteToInt( recordData[11] ) );
        setSensorDirection( SensorDirection.parseInt( ( TypeConverter.byteToInt( recordData[12] ) & 0xc0 ) >> 6 ) );
        setIdInstanceModifierType( InstanceModifierType.parseInt( ( TypeConverter.byteToInt( recordData[12] )
            & 0x30 ) >> 4 ) );

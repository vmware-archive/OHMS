    ...
    extends FruRecord
{
    ...
    public ChassisInfo( byte[] fruData, int offset )
    {
        ...
        chassisType = ChassisType.parseInt( TypeConverter.byteToInt( fruData[offset + 2] ) );
        ...
        while ( partNumber != 0xc1 && offset < fruData.length )
        {
            ...
            if ( partDataLength > 0 && partDataLength + offset < fruData.length )
            {
                ...
                {
                    case 0:
                        setChassisPartNumber( FruRecord.decodeString( partType, partNumberData, true ) );
                        break;
                    case 1:
                        setChassisSerialNumber( FruRecord.decodeString( partType, partNumberData, true ) );
                        break;
                    default:
                        if ( partDataLength == 0 )
                        {

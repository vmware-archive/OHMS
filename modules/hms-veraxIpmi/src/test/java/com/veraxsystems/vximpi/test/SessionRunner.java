    ...
    public void run()
    {
        ...
        for ( int i = 0; i < 1; ++i )
        {
            ...
            try
            {
                ...
                Thread.sleep( Math.abs( Randomizer.getInt() ) % 200 );
                connection.getChannelAuthenticationCapabilities( tag, cs, PrivilegeLevel.User );
                Thread.sleep( Math.abs( Randomizer.getInt() ) % 200 );
                connection.startSession( tag, cs, PrivilegeLevel.User, properties.getProperty( "username" ),

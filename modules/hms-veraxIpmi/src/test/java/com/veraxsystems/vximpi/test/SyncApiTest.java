...
public class SyncApiTest
{
     ...
     */
    @Test
    public void testGetAvailableCipherSuites()
        throws Exception
    {
    ...
    /**
     * Tests {@link IpmiConnector#getChannelAuthenticationCapabilities(ConnectionHandle, CipherSuite, PrivilegeLevel)}
     *
     * @throws Exception
     */
    @Test
    public void testGetChannelAuthenticationCapabilities()
        throws Exception
    {
        logger.info( "Testing GetAvailableCipherSuites" );
    ...
    /**
     * Tests {@link IpmiConnector#openSession(ConnectionHandle, String, String, byte[])}
     *
     * @throws Exception
     */
    @Test
        ...
        throws Exception
    {
        ...
        connector.openSession( connection, properties.getProperty( "username" ), properties.getProperty( "password" ),
                               null );
    }
        ...
        throws Exception
    {
        ...
        testOpenSession();
        connector.sendMessage( connection, new GetChassisStatus( IpmiVersion.V20, connection.getCipherSuite(),
                                                                 AuthenticationType.RMCPPlus ) );
    }

    @Test
    public void testSdrPartialRead()
        throws Exception
    {
        logger.info( "Testing SDR partial read" );
        testOpenSession();
        int reservationId =
            ( (ReserveSdrRepositoryResponseData) connector.sendMessage( connection,
                                                                        new ReserveSdrRepository( IpmiVersion.V20,
                                                                                                  connection.getCipherSuite(),
                                                                                                  AuthenticationType.RMCPPlus ) ) ).getReservationId();
        GetSdrResponseData data1 =
            (GetSdrResponseData) connector.sendMessage( connection,
                                                        new GetSdr( IpmiVersion.V20, connection.getCipherSuite(),
                                                                    AuthenticationType.RMCPPlus, reservationId, 0 ) );
        byte[] sensorData = data1.getSensorRecordData();
        GetSdrResponseData data2 =
            (GetSdrResponseData) connector.sendMessage( connection,
                                                        new GetSdr( IpmiVersion.V20, connection.getCipherSuite(),
                                                                    AuthenticationType.RMCPPlus, reservationId, 0, 0,
                                                                    sensorData.length / 2 ) );
        GetSdrResponseData data3 =
            (GetSdrResponseData) connector.sendMessage( connection,
                                                        new GetSdr( IpmiVersion.V20, connection.getCipherSuite(),
                                                                    AuthenticationType.RMCPPlus, reservationId, 0,
                                                                    sensorData.length / 2,
                                                                    sensorData.length - sensorData.length / 2 ) );
        ...
        System.arraycopy( data3.getSensorRecordData(), 0, merged, data2.getSensorRecordData().length,
                          data3.getSensorRecordData().length );

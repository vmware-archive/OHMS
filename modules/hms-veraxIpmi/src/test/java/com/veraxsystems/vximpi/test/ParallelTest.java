...
public class ParallelTest
{
        ...
        for ( int i = 0; i < CONNECTION_COUNT; ++i )
        {
            if ( i % 2 == 0 )
            {
                ConnectionHandle connection =
                    connector.createConnection( InetAddress.getByName( properties.getProperty( "testIp" ) ) );
                connections.add( connection );
            }
            else
            {
                ConnectionHandle connection =
                    connector.createConnection( InetAddress.getByName( properties.getProperty( "testIp2" ) ) );
                connections.add( connection );
            }
        }
        succeeded = true;
    ...
    public void testGetAvailableCipherSuites()
        throws Exception
    {
        System.out.println( "[JUnit] [SyncApiTest] Testing GetAvailableCipherSuites" );
        for ( ConnectionHandle handle : connections )
        {
    ...
    {
        System.out.println( "[JUnit] [SyncApiTest] Testing GetAvailableCipherSuites" );
        System.out.println( "[JUnit] [SyncApiTest] Testing GetChannelAuthenticationCapabilities" );
        for ( ConnectionHandle handle : connections )
        {
        ...
        implements Runnable
    {
        ...
        public void run()
        {
            ...
            try
            {
                for ( int i = 0; i < cnt; ++i )
                {
                    GetChassisStatusResponseData responseData =
                        (GetChassisStatusResponseData) connector.sendMessage( connection,
                                                                              new GetChassisStatus( IpmiVersion.V20,
                                                                                                    connection.getCipherSuite(),
                                                                                                    AuthenticationType.RMCPPlus ) );
                    logger.info( "[" + id + "] Received message " + msgCnt++ + " from " + id + ": "

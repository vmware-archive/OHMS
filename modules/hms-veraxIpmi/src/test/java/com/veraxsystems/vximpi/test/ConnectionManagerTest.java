    ...
    extends TestCase
{
    ...
    /**
     * Tests sending multiple parallel sessionless messages and providng them proper tags.
     */
    @Test
    public void testOverload()
    {
        ...
        for ( int i = 0; i < runs; ++i )
        {
            ...
            try
            {
                or = new OverloadRunner( i, InetAddress.getByName( properties.getProperty( "testIp" ) ) );
            }
        ...
        logger.info( "Packets sent: " + UdpMessenger.getSentPackets() );
        assertEquals( (Integer) 0, finishedCorrectly );
    }

    /**
     * Tests message timeout. After a timeout message should is removed form {@link MessageQueue} and error message is
     * returned.
     */
    @Test
    public void testTimeout()
    {
        ...
        for ( int i = 0; i < runs; ++i )
        {
            ...
            try
            {
                or = new OverloadRunner( i, InetAddress.getByName( "192.168.127.200" ) );
            }
            catch ( UnknownHostException e )
            {
                e.printStackTrace();
        ...
        throws FileNotFoundException, IOException
    {
        ...
        try
        {
            or = new OverloadRunner( 0, InetAddress.getByName( properties.getProperty( "testIp" ) ) );
        }
        catch ( UnknownHostException e )
        {
            e.printStackTrace();
        ...
        implements ConnectionListener
    {
        ...
        public void run()
        {
            ...
            logger.info( "Process " + this.index + " - time spent sending: " + ( new Date().getTime() - d.getTime() ) );
        }
            ...
            throws FileNotFoundException, IOException
        {
            ...
            try
            {
                manager.getChannelAuthenticationCapabilities( index, cs, PrivilegeLevel.User );
            }
            catch ( Exception e )
            {
                e.printStackTrace();
            ...
            try
            {
                manager.startSession( index, cs, PrivilegeLevel.User, properties.getProperty( "username" ),
            ...
            for ( int i = 0; i < count; ++i )
            {
                ...
                {
                    int result =
                        c.sendIpmiCommand( new GetChassisStatus( IpmiVersion.V20, cs, AuthenticationType.RMCPPlus ) );
                    if ( result < 0 )
                    {
                        Thread.sleep( 10 );
                        --i;
        ...
        public void notify( ResponseData responseData, int handle, int tag, Exception exception )
        {
            ...
            else
            {
                ...
                logger.info( "Received answer " + tag + " : " + responseData.getClass().getSimpleName() );
            }
        }
    }
}

...
/*
 * BoardServiceTest class is dummy class to test User Story S104329
 * NIC and Storage Controller Events needs to be generated if the last status for port and Storage Controller respectively was different than the current one.
 *
 */
    ...
    implements IBoardService
{
    ...
    static int countForStorageControllerEventEndPointHit = 1;
        ...
        throws HmsException
    {
        ...
        switch ( component )
        {
            ...
            case STORAGE_CONTROLLER:
                ServerComponentEvent storageControllerEvent = new ServerComponentEvent();
                if ( countForStorageControllerEventEndPointHit % 3 == 0 )
                {
                    storageControllerEvent.setEventName( NodeEvent.STORAGE_CONTROLLER_DOWN );
                    storageControllerEvent.setEventId( "Storage Controller down" );
                    storageControllerEvent.setComponentId( "STORAGE_CONTROLLER1" );
                    storageControllerEvent.setUnit( EventUnitType.DISCRETE );
                    storageControllerEvent.setDiscreteValue( "STORAGE_CONTROLLER1 down" );
                    events.add( storageControllerEvent );
                }
                else
                {
                    storageControllerEvent = new ServerComponentEvent();
                    storageControllerEvent.setEventName( NodeEvent.STORAGE_CONTROLLER_UP );
                    storageControllerEvent.setEventId( "Storage Controller up" );
                    storageControllerEvent.setComponentId( "STORAGE_CONTROLLER1" );
                    storageControllerEvent.setUnit( EventUnitType.DISCRETE );
                    storageControllerEvent.setDiscreteValue( "STORAGE_CONTROLLER1 up" );
                    events.add( storageControllerEvent );
                }
                countForStorageControllerEventEndPointHit++;
                break;
            case NIC:
                ...
                {
                    nicEvent = new ServerComponentEvent();
                    nicEvent.setEventName( NodeEvent.NIC_PORT_UP );
                    nicEvent.setEventId( "NIC 1 port up" );
                    nicEvent.setComponentId( "NIC1" );
                    nicEvent.setUnit( EventUnitType.DISCRETE );
                    nicEvent.setDiscreteValue( "NIC 1 port up" );
                    events.add( nicEvent );
                }
                countForNicEventEndPointHit++;
                nicEvent = new ServerComponentEvent();
